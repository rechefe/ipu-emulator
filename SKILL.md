...
| ACC | `ACC.ADD`, `ACC.ADD.FIRST`, `ACC.MAX`, `ACC.MAX.FIRST`, `ACC.SUB`, `ACC.SUB.FIRST`, `ACC.STRIDE`, `AGG.SUM`, `AGG.SUM.FIRST`, `AGG.MAX`, `AGG.MAX.FIRST`, `RESHAPE` | Accumulate into `R_ACC`; AGG instructions reduce `MULT_RES` elements (sum/max) into a single slot of `R_ACC`; `RESHAPE` permutes `R_ACC` word elements using two `LrdIdx` byte-index arrays gated by `reshape_mask` |
