...
### `mask_shift` — sequential shift-and-AND index

The value in the **`mask_shift`** LR register is interpreted as a signed index **`mask_shift_idx` ∈ [−3, +3]** (values outside this range are clamped; LR registers are 20-bit so negative indices use 20-bit two's complement). It selects from seven masks generated by sequentially shifting the base mask and ANDing with the **partition vector** at each step:

| `mask_shift_idx` | Derived mask |
|---|---|
| `0` | base mask from `R_MASK[mask_offset]` (unmodified) |
| `−1` | `(base >> 1) & partition_vector` |
| `−2` | `(mask[−1] >> 1) & partition_vector` |
| `−3` | `(mask[−2] >> 1) & partition_vector` |
| `+1` | `(base << 1) & partition_vector` |
| `+2` | `(mask[+1] << 1) & partition_vector` |
| `+3` | `(mask[+2] << 1) & partition_vector` |

**Partition vector** — a 128-bit value derived from `CR15.partition`:

- `partition = 0`: no partitioning — partition vector is all-ones (shifts are unconstrained).
- `partition = P` (`P > 0`): P partitions of `128/P` elements each. A **0** bit marks the start of each partition; **1** bits mark elements within a partition. Example for `P = 2`: `0 1`⁶³` 0 1`⁶³.

The AND at each step prevents mask bits from crossing partition boundaries. Lanes in **`mult_res`** where the resulting mask bit is **1** are zeroed before accumulation.
