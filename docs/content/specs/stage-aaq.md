...
functions to the 128-lane accumulator and/or quantizes the accumulator into an
FP8/INT8 vector for output. It produces:

- A 128-element `post_aaq_reg` vector via `ACTIVATE` (FP32 lanes after activation).
- A 128-byte `aaq_result` vector via `AAQ` (any 8-bit format: INT8 or FP8 e(x)m(7-x)).

> **Note:** Aggregation (reduce sum / reduce max over `R_ACC` lanes) has moved
> to the **ACC slot** — see `AGG.SUM`, `AGG.SUM.FIRST`, `AGG.MAX`,
> `AGG.MAX.FIRST` in the ACC stage spec.
...
| `op` | `input logic [1:0]` | Selects the AAQ operation: `aaq_nop`=0, `aaq`=1, `activate`=2. Sampled only when `valid`=1. |
...
**Assembly syntax:** `ACTIVATE activation_fn full_xmem_row`
...
### 7.1 Quantize (`AAQ`) Work in progress

Requires INT8 mode (`dtype == DType.INT8`). Takes no operands. Reads `POST_AAQ_REG`
lanes as FP32, quantizes to INT8, clamps, and writes the 128-byte result to
`aaq_result`.

```text
// dtype must be DType.INT8
...
### 8.2 `ACTIVATE` — Apply Activation Function

- **Summary:** Apply an element-wise activation function to `r_acc[0..n-1]` and write the results to `POST_AAQ_REG`. `full_xmem_row` controls the active lane count.
- **Syntax:** `ACTIVATE activation_fn full_xmem_row`
- **Operands:**
  - `activation_fn` — the activation function to apply (see §7.0 table).
...
- **Operation:**
  ...
  aaq_result[n..127] = 0
  ```
  The Quantization block appends 12 bits of metadata to the 1024-bit payload (see §7.1).
- **Example:** `AAQ 1;;` (full row), `AAQ 0;;` (use `CR15.valid_elements`).
- **Notes:** `aaq_result` must be flushed with `XMEM.STORE_AAQ_RESULT offset base` before the next `AAQ` overwrites it (see §7.1).
