...
- `CR15` is reserved for the dstructure configuration register (`valid_elements` in bits `[7:0]`, `partition` in bits `[11:8]`) and must not be used for application data. The Python emulator stores dtype on `IpuState.dtype`, not in a `CR` register.
