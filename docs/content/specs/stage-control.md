...
- **Contents:** every entry is an **already-decoded** VLIW word. Slot fields (cond, three LR sub-slots, LOAD, MULT, ACC, AAQ, STORE, ACC_STORE) are laid out explicitly in the entry — CTRL only demultiplexes them, it does not perform any opcode-level decode. The **acc_store** slot (`STR_ACC_REG`) is simulation-only and not implemented in real IPU hardware.
