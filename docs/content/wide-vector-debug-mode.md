...
- **`ldr_mult_reg`** reads **512 bytes** from XMEM (128×FP32 or 128×INT32, depending on `wide_vector_arithmetic`) into internal staging for `r0`, `r1`, or `mem_bypass`. The architectural 128-byte `r` register bytes in the regfile are not the source for mult operands in this mode. **`MULT.EE`** only accepts **`r0`/`r1`**; `mem_bypass` remains valid as a **`ldr_mult_reg`** destination for same-cycle staging patterns that do not use **`MULT.EE`** with encoding `2`.
