...
\texttt{CR0}--\texttt{CR15} & 16 $\times$ 32-bit & control registers, set before the run \\
\bottomrule
\end{tabularx}
\end{center}
...
The emulator runs the slots of a word in this order: the three \texttt{lr}
...
LR address offsets of \texttt{load}, \texttt{mult}, \texttt{store} & this
...
\texttt{ACC.ADD.FIRST} in one word, and \texttt{ACTIVATE.QUANTIZE} and
\texttt{STR\_POST\_AAQ\_REG} in another, but loads \texttt{R0} one word
...
The repository contains the assembler, the emulator and the kernel library:
...
options of the kernel (section~\ref{sec:args}). Section~\ref{sec:newkernel}
builds a complete kernel, file by file.
...
come from this kernel's \texttt{cases.py}; other kernels have their own.
...
A shape the kernel does not support is refused before anything runs:
...
\textbf{alias}. The \doc{isa-alias-catalogue}{ISA Alias Catalogue} lists them as
A1--A16; counting them shows which new instructions would help most.
