...
void ipu_setup(ipu__obj_t *ipu, int argc, char **argv)
{
    ...
    LOG_INFO("Setting up IPU for 128×256 fully connected layer...");
    ...
    // Load input activations from file (1 sample × 128 bytes = 128 bytes)
    int inputs_loaded = emulator__load_binary_to_xmem(
        ipu->xmem, inputs_file, INPUT_BASE_ADDR, INPUT_SIZE, SAMPLES_NUM);
    ...
    // Load weights from file (128 elements × 256 neurons = 32768 bytes)
    // Each weight row is 256 bytes (one R register)
    int weights_loaded = emulator__load_binary_to_xmem(
        ipu->xmem, weights_file, WEIGHTS_BASE_ADDR, OUTPUT_NEURONS, INPUT_SIZE);
    ...
    // Load zeros for clearing accumulator registers (1024 bytes = 2 RQ registers)
    const char *zeros_file = "src/apps/fully_connected_ZD/128x256/zeros_1024_bytes.bin";
    int zeros_loaded = emulator__load_binary_to_xmem(
        ipu->xmem, zeros_file, ZEROS_BASE_ADDR, IPU__RQ_REG_SIZE_BYTES * 2, 1);
    ...
    LOG_INFO("Configuration: %d sample × %d inputs → %d neurons", SAMPLES_NUM, INPUT_SIZE, OUTPUT_NEURONS);
}
...
void ipu_teardown(ipu__obj_t *ipu, int argc, char **argv)
{
    ...
    // Save output activations to file (256 neurons × 4 bytes = 1024 bytes = 2 RQ registers)
    int outputs_saved = emulator__dump_xmem_to_binary(
        ipu->xmem, outputs_file, OUTPUT_BASE_ADDR, OUTPUT_NEURONS * sizeof(int32_t), SAMPLES_NUM);
...
int main(int argc, char **argv)
{
    emulator__test_config_t config = {
        .test_name = "IPU Fully Connected Layer 128×256 (batch=1)",
        .max_cycles = 1000000,
