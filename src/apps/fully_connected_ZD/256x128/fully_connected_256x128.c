...
void ipu_setup(ipu__obj_t *ipu, int argc, char **argv)
{
    ...
    LOG_INFO("Setting up IPU for 256×128 fully connected layer...");
    ...
    // Load input activations from file (1 sample × 256 bytes = 256 bytes = 2 R registers)
    ...
    // Load weights from file (256 elements × 128 neurons = 32768 bytes)
    // Each weight row is 128 bytes (one R register)
    ...
    // Load zeros for clearing accumulator registers (512 bytes = 1 RQ register)
    const char *zeros_file = "src/apps/fully_connected_ZD/256x128/zeros_512_bytes.bin";
...
int main(int argc, char **argv)
{
    emulator__test_config_t config = {
        .test_name = "IPU Fully Connected Layer 256×128 (batch=1)",
        .max_cycles = 1000000,
