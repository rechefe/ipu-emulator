#!/usr/bin/env python3
"""
Generate test data for fully connected layer: 256 inputs × 128 output neurons, batch size = 1.

This script creates:
1. Input matrix: 1 sample × 256 elements (int8)
2. Weight matrix: 256 input elements × 128 neurons (int8) - TRANSPOSED layout
3. Expected output: 1 sample × 128 neurons (int32)
...
def main():
    print("=" * 70)
    print("Generating test data for 256×128 fully connected layer (batch=1)")
    print("=" * 70)

    # Generate inputs
    inputs = generate_inputs()
    save_int8_binary(inputs, "inputs_1x256_int8.bin")

    # Generate weights in transposed layout
    weights_transposed = generate_weights_transposed()
    save_int8_binary(weights_transposed, "weights_256x128_int8.bin")

    # Compute expected output
    expected_output = compute_expected_output(inputs, weights_transposed)
    save_int32_binary(expected_output, "output_1x128_int32.bin")

    print("\n" + "=" * 70)
    print("Test data generation complete!")
    print("=" * 70)
    print("\nFiles created:")
    print(f"  1. inputs_1x256_int8.bin       - {NUM_SAMPLES} sample × {INPUT_SIZE} elements = {NUM_SAMPLES * INPUT_SIZE} bytes")
    print(f"  2. weights_256x128_int8.bin    - {INPUT_SIZE} elements × {NUM_NEURONS} neurons = {INPUT_SIZE * NUM_NEURONS} bytes (TRANSPOSED)")
    print(f"  3. output_1x128_int32.bin      - {NUM_SAMPLES} sample × {NUM_NEURONS} neurons = {NUM_SAMPLES * NUM_NEURONS * 4} bytes")
