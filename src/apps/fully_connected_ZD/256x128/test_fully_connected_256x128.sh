#!/bin/bash
# Test script for IPU Fully Connected Layer 256×128 (batch=1)
...
echo "Running IPU Fully Connected Layer 256×128 Test (batch=1)..."
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Test PASSED: IPU Fully Connected Layer 256×128 executed successfully"
    echo "Output saved to: $OUTPUTS_FILE"
    exit 0
else
    echo "Test FAILED: IPU Fully Connected Layer 256×128 returned exit code $EXIT_CODE"
    exit 1
fi
