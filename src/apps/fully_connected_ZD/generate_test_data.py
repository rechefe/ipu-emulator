#!/usr/bin/env python3
"""
Generate test data for fully connected layer with parallel neuron computation.

This script creates:
1. Input matrix: 10 samples × 128 elements (int8)
2. Weight matrix: 128 input elements × 128 neurons (int8) - TRANSPOSED layout
3. Expected output: 10 samples × 128 neurons (int32)
...
def generate_inputs():
    ...
    print(f"\nGenerated inputs: {NUM_SAMPLES} samples × {INPUT_SIZE} elements")
...
def compute_expected_output(inputs, weights_transposed):
    ...
    print(f"\nComputed expected output: {NUM_SAMPLES} samples × {NUM_NEURONS} neurons")
...
def main():
    print("=" * 70)
    print("Generating test data for parallel fully connected layer")
    print("=" * 70)

    # Generate inputs
    inputs = generate_inputs()
    save_int8_binary(inputs, "inputs_10x128_int8.bin")

    # Generate weights in transposed layout
    weights_transposed = generate_weights_transposed()
    save_int8_binary(weights_transposed, "weights_128x128_int8.bin")

    # Compute expected output
    expected_output = compute_expected_output(inputs, weights_transposed)
    save_int32_binary(expected_output, "output_10x128_int32.bin")

    print("\n" + "=" * 70)
    print("Test data generation complete!")
    print("=" * 70)
    print("\nFiles created:")
    print(f"  1. inputs_10x128_int8.bin      - {NUM_SAMPLES} samples × {INPUT_SIZE} elements = {NUM_SAMPLES * INPUT_SIZE} bytes")
    print(f"  2. weights_128x128_int8.bin    - {INPUT_SIZE} elements × {NUM_NEURONS} neurons = {INPUT_SIZE * NUM_NEURONS} bytes (TRANSPOSED)")
    print(f"  3. output_10x128_int32.bin     - {NUM_SAMPLES} samples × {NUM_NEURONS} neurons = {NUM_SAMPLES * NUM_NEURONS * 4} bytes")
