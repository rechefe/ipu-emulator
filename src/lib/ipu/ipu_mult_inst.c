                                      ...
                                      const ipu__regfile_t *regfile_snapshot)
{
    ...
    uint32_t lr_fixed_cyclic_idx = inst.mult_inst_token_2_lr_reg_field;
    uint32_t lr_fixed_cyclic_val = regfile_snapshot->lr_regfile.lr[lr_fixed_cyclic_idx];

    ipu__r_reg_t *ra_reg_ptr = ipu__get_mult_stage_r_reg(ipu, ra_idx);
    ipu__r_reg_t rb_reg;

    ipu__get_r_cyclic_at_idx(ipu, lr_fixed_cyclic_val, &rb_reg);
...
void ipu__execute_mult_ve_instruction(ipu__obj_t *ipu,
                                      inst_parser__inst_t inst,
                                      const ipu__regfile_t *regfile_snapshot)
{
    inst_parser__mult_stage_reg_field_t ra_idx =
    ...
    uint32_t lr_cyclic_idx = inst.mult_inst_token_2_lr_reg_field;
    uint32_t lr_cyclic_val = regfile_snapshot->lr_regfile.lr[lr_cyclic_idx];

    uint32_t lr_fixed_ra_idx = inst.mult_inst_token_5_lr_reg_field;
    uint32_t lr_fixed_ra_val = regfile_snapshot->lr_regfile.lr[lr_fixed_ra_idx];

    ipu__r_reg_t *ra_reg_ptr = ipu__get_mult_stage_r_reg(ipu, ra_idx);
    ipu__r_reg_t rb_reg;

    ipu__get_r_cyclic_at_idx(ipu, lr_cyclic_val, &rb_reg);

    for (int i = 0; i < IPU__R_REG_SIZE_BYTES; i++)
    {
        ipu_math__mult(
            &ra_reg_ptr->bytes[lr_fixed_ra_val % IPU__R_REG_SIZE_BYTES],
            &rb_reg.bytes[i],
            &ipu->misc.mult_res.words[i],
            ipu__get_cr_dtype(&ipu->regfile));
    }
                                      ...
                                      const ipu__regfile_t *regfile_snapshot)
{
    ...
    uint32_t lr_cyclic_offset_idx = inst.mult_inst_token_2_lr_reg_field;
    uint32_t lr_cyclic_offset_val = regfile_snapshot->lr_regfile.lr[lr_cyclic_offset_idx];

    ipu__r_reg_t *ra_reg_ptr = ipu__get_mult_stage_r_reg(ipu, ra_idx);
    ipu__r_reg_t rb_reg;

    ipu__get_r_cyclic_at_idx(ipu, lr_cyclic_offset_val, &rb_reg);
                                   ...
                                   const ipu__regfile_t *regfile_snapshot)
{
    ...
    case INST_PARSER__MULT_INST_OPCODE_MULT_VE:
    {
        ipu__execute_mult_ve_instruction(ipu, inst, regfile_snapshot);
        break;
    }
