...
{
    // Store ACC register to external memory: STR ACC, [LR, CR]
    inst_parser__acc_stage_reg_field_t acc_stage_idx =
        (inst_parser__acc_stage_reg_field_t)inst.xmem_inst_token_2_acc_stage_reg_field;

    // Currently stores ACC stage to XMEM at address LR + CR
    int lr_idx = inst.xmem_inst_token_3_lr_reg_field;
    int cr_idx = inst.xmem_inst_token_5_cr_reg_field;
    ...
    uint8_t *acc_value_ptr;
    uint32_t acc_value_size;

    ipu__get_acc_reg_by_enum(
        ipu->regfile.acc_stage_regfile.r_acc,
        acc_stage_idx,
        &acc_value_ptr,
        &acc_value_size);

    xmem__load_array_to(ipu->xmem, acc_value_ptr, acc_value_size, target_address);
}
...
static void ipu__execute_xmem_ldr_mult_reg(ipu__obj_t *ipu, inst_parser__inst_t inst, const ipu__regfile_t *regfile_snapshot)
{
    ...
    case INST_PARSER__MULT_STAGE_REG_FIELD_R0:
        r_reg_to_load_to = &ipu->regfile.mult_stage_regfile.r0;
        break;
    case INST_PARSER__MULT_STAGE_REG_FIELD_R1:
        r_reg_to_load_to = &ipu->regfile.mult_stage_regfile.r1;
        break;
...
{
    int lr_offset_idx = inst.xmem_inst_token_4_lr_reg_field;
    int cr_offset_idx = inst.xmem_inst_token_5_cr_reg_field;
