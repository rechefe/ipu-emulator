"""Debug runner for attn_scores_km_16x60.
...
``test/test_attn_scores_km_16x60_wide.py`` is the reference for correctness.

Usage::

    ATTN_SCORES_KM_16X60_INST_BIN=/tmp/attn_scores_km_16x60.bin \
    uv run python -m ipu_apps.attention.attn_scores_km_16x60
"""
...
from ipu_apps.attention.attn_scores_km_16x60.gen_debug_data import generate


def main() -> None:
    _INST_BIN = Path(os.environ["ATTN_SCORES_KM_16X60_INST_BIN"])

    work = Path(tempfile.mkdtemp(prefix="attn_scores_km_16x60_"))
