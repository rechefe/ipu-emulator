"""FP32 debug-input generation for attn_scores_km_16x60.
...
``test/test_attn_scores_km_16x60_wide.py`` so that
``python -m ipu_apps.attention.attn_scores_km_16x60`` exercises the kernel the way its
test does.
...
from ipu_apps.attention.attn_scores_km_16x60 import N_TOK, D, N_HEADS
...
def generate(out_dir: Path) -> dict[str, Path]:
    ...
    # Canonical channel-major files spanning all heads:
    # element [token t, channel h*D+c] at (h*D + c)*N_TOK + t.
    n_chan = N_HEADS * D
    Q = rng.uniform(-1.0, 1.0, size=(n_chan, N_TOK))
    K = rng.uniform(-1.0, 1.0, size=(n_chan, N_TOK))
    return {
