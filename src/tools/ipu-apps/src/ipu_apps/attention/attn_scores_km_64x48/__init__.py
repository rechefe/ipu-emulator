...
# (n_tok=64, d=48). `head` stays OUT of `requires`/`supports` for the same
# reason as the sibling apps: it selects which of the N_HEAD heads already in
# the input file to compute, not a shape the query routes on -- the kernel
# equally supports any head in range regardless of which one a given query
# will eventually request. The `0 <= head < N_HEAD` bound is therefore left as
# a pure constructor invariant (already enforced there) rather than
# duplicated in `supports`: it is validated once `head` is actually chosen at
# construction time, not at routing time when it may not even be known yet.
...
SPEC = KernelSpec(
    ...
    op="attn_scores_km",
    variant="64x48",
    app_class=AttnScoresKM64x48App,
    asm="attn_scores_km_64x48.asm",
    requires=("n_tok", "d"),
