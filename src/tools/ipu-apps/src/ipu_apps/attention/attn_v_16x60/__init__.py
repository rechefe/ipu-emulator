...
# (n_tok=16, d=60). The relocatable pbase_row/vbase_row/obase_row kwargs (and
# the p_path=None/v_path=None chained-kernel skip) are placement/staging
# concerns, not shape the registry routes on, so `build` leaves them at their
# constructor defaults -- a caller that needs a non-default layout still
...
SPEC = KernelSpec(
    ...
    op="attn_v",
    variant="16x60",
    app_class=AttnV16x60App,
    asm="attn_v_16x60.asm",
    requires=("n_tok", "d"),
