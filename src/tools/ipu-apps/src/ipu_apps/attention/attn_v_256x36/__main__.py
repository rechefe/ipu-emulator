"""Debug runner for attn_v_256x36.
...
``test/test_attn_v_256x36_wide.py`` is the reference for correctness.

Usage::

    ATTN_V_256X36_INST_BIN=/tmp/attn_v_256x36.bin \
    uv run python -m ipu_apps.attention.attn_v_256x36
"""
...
from ipu_apps.attention.attn_v_256x36.gen_debug_data import generate

_INST_BIN = Path(os.environ["ATTN_V_256X36_INST_BIN"])


def main() -> None:
    work = Path(tempfile.mkdtemp(prefix="attn_v_256x36_"))
