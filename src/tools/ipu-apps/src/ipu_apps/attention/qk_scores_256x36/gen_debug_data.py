"""FP32 debug-input generation for qk_scores_256x36.
...
``test/test_qk_scores_256x36_wide.py`` so that ``python -m ipu_apps.attention.qk_scores_256x36`` exercises the
...
from ipu_apps.attention.qk_scores_256x36 import N, D
...
def generate(out_dir: Path) -> dict[str, Path]:
    ...
    Q = rng.uniform(-1.0, 1.0, size=(D, N))
    K = rng.uniform(-1.0, 1.0, size=(D, N))
    return {
