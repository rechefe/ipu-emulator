"""2-channel convolution harness (128x128x2 input, 3x3x2 kernel, 128x128x1 output).

Mirrors the depthwise convolution harness pattern:
1. Loads input image (2 channels interleaved by row), kernel weights, and mask data into XMEM
...
class Conv2chApp(IpuApp):
    """2-channel convolution application harness.

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input image binary (128x128x2 bytes, interleaved by row).
        kernel_path: Path to kernel binary (18 bytes, padded to 128).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load kernel (padded to 128 bytes, only first 18 matter)
