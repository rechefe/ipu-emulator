"""Debug runner for the 2-channel to 2-channel convolution app.

Usage::

    bazel run //src/tools/ipu-apps:conv2ch2ch -- --dtype INT8
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run 2ch->2ch conv with debug CLI")
    ...
    inst_path = Path(os.environ["CONV2CH2CH_INST_BIN"])
    data_dir = Path(os.environ["CONV2CH2CH_DATA_DIR"])
