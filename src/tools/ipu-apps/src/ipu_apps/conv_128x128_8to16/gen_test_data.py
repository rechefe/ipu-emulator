"""Generate test data for the standard 8->16 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

128x128 spatial, 8 input channels, 16 output channels, 3x3 kernels.
"""
...
def _reference_conv_8to16(
    input_bytes: bytes, kernel_data: list[bytes], dtype: DType
) -> bytes:
    """Compute the reference standard convolution output (8 -> 16 channels).
    ...
    Kernel layout per filter (72 bytes):
    ...
    Returns 128*128*16 accumulator words packed as bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    golden = _reference_conv_8to16(input_bytes, kernel_filters, dtype)
