"""Standard 3x3 convolution harness (256 input -> 128 output channels, 16x16).

16x16 spatial, 8 rows packed per 128-byte chunk (2 groups).
Each output channel has 256 input-channel 3x3 kernels (2304 bytes total).
Since 2304 > 128, each filter is stored as 32 x 128-byte blocks:
  block 0:  input channels 0-7     (72 bytes + 56 padding)
  block 1:  input channels 8-15    (72 bytes + 56 padding)
  ...
  block 31: input channels 248-255 (72 bytes + 56 padding)
"""
...
TAPS_PER_FILTER = IN_CHANNELS * KERNEL_SIZE  # 2304

ROWS_PER_CHUNK = 128 // COLS  # 8
NUM_GROUPS = ROWS // ROWS_PER_CHUNK  # 2

INPUT_BASE_ADDR = 0x000000
KERNEL_BASE_ADDR = 0x010000
OUTPUT_BASE_ADDR = 0x090000
MASK_BASE_ADDR = 0x0B0000

# Each filter: 32 blocks x 128 bytes = 4096 bytes
CHANNELS_PER_BLOCK = 8
BLOCKS_PER_FILTER = IN_CHANNELS // CHANNELS_PER_BLOCK  # 32
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * 128  # 4096
KERNEL_TOTAL_BYTES = OUT_CHANNELS * FILTER_PADDED_BYTES  # 524288
...
def _build_mask_data() -> bytes:
    ...
    Layout (8 slots of 16 bytes = 128 bits each, 8 rows x 16 cols packed):
      slot 0: all zeros                          -- no masking (kc=0)
      slot 1: bits {0, 16, 32, 48, 64, 80, 96, 112} -- zero col 0 of each row (kc=-1)
      slot 2: bits {15, 31, 47, 63, 79, 95, 111, 127} -- zero col 15 of each row (kc=+1)
      slot 3: bits {112..127}                    -- zero bottom row (last group kr=+1, kc=0)
      slot 4: bits {0,16,32,48,64,80,96,112..127} -- left + bottom (last group kr=+1, kc=-1)
      slot 5: bits {15,31,47,63,79,95,111..127}  -- right + bottom (last group kr=+1, kc=+1)
    ...
    # Slot 1: left border -- col 0 of each of 8 rows
    for row in range(8):
        set_bit(1, row * 16)

    # Slot 2: right border -- col 15 of each of 8 rows
    for row in range(8):
        set_bit(2, row * 16 + 15)

    # Slot 3: bottom row of chunk (row 7, positions 112-127)
    set_bit_range(3, 112, 127)

    # Slot 4: left border + bottom row
    for row in range(8):
        set_bit(4, row * 16)
    set_bit_range(4, 112, 127)

    # Slot 5: right border + bottom row
    for row in range(8):
        set_bit(5, row * 16 + 15)
    set_bit_range(5, 112, 127)

    return bytes(mask)


def _build_kernel_data(kernel_raw: bytes) -> bytes:
    """Pack kernel into 128 filters x 32 blocks x 128 bytes.

    Input: 294912 bytes (128 filters x 256 input channels x 9 taps, contiguous).
      kernel_raw[f * 2304 + ic * 9 + tap]

    Output: 524288 bytes (128 filters x 32 blocks x 128 bytes).
...
class Conv16x16x256to128App(IpuApp):
    """Standard 3x3 convolution application harness (256->128 channels, 16x16).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (2 groups x 256 ch x 128 bytes = 65536 bytes).
        kernel_path: Path to kernel binary (294912 bytes = 128 x 256 x 9 taps, raw).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (2 groups x 256 channels x 128 bytes = 65536 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (294912 bytes raw -> padded to 524288 bytes)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 2 groups x 128 output channels = 256 chunks of 512 bytes
