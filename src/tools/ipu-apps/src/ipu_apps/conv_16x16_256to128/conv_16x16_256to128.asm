# Standard 3x3 convolution: 256 input -> 128 output channels, 16x16.
#
...
#
# Each output channel has 256 input-channel 3x3 kernels (full cross-channel
# mixing).  Kernel per filter: 256 * 9 = 2304 bytes, which exceeds r0's
# 128 bytes.  Solution: split into 32 input-channel groups of 8, each
# fitting in one r0 load (72 bytes + 56 padding = 128 bytes per block).
#
# Memory layout (set via CR registers):
#   cr0 = input  base  (2 groups x 256 ch x 128 bytes = 65536 bytes)
#   cr1 = kernel base  (128 filters x 32 blocks x 128 bytes = 524288 bytes)
...
#
# Kernel layout (128 filters x 32 blocks x 128 bytes = 524288 bytes):
#   Filter f, block b (0..31):
#     At XMEM offset (f * 32 + b) * 128 from kernel base.
...
#   lr6  = 524288 (filter loop limit = 128 filters x 4096 bytes/filter)
#   lr7  = (unused permanent)
#   lr8  = 1     (mask_slot_1 = kc=-1)
#   lr9  = 2     (mask_slot_2 = kc=+1)
#   lr10 = channel offset (0, 128, ..., 32640, spans all 32 channel groups)
#   lr11 = 1024  (input-channel group size = 8 ch x 128 bytes)
#   lr12 = temp  (S1 address)
#   lr13 = channel loop end (lr10 + 1024)
#   lr14 = kernel memory offset (0, 128, ..., 524160)
#   lr15 = 32768 (row-group stride = 256 x 128, also total channels x 128)
#
# Structure:
#   Each filter needs 32 r0 loads (8 input channels each).
...
# ===========================================================================
    ...
    # lr15 = 32768 (group stride, exceeds set max of 32767)
    add                 lr15 lr15 lr15;;
