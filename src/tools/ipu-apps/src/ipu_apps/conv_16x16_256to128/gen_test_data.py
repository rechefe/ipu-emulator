"""Generate test data for the 16x16 256->128 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

16x16 spatial, 256 input channels, 128 output channels, 3x3 kernel.
8 spatial rows packed per 128-byte chunk (2 row-groups).
...
def _reference_conv_16x16_256to128(
    input_bytes: bytes, kernel_bytes: bytes, dtype: DType
) -> bytes:
    """Compute the reference standard convolution output (256->128, 16x16).

    For each output filter f, output pixel (r, c):
      acc = sum over ic, dr, dc:
              kernel[f*2304 + ic*9 + dr*3 + dc] * input[ic][r+dr-1][c+dc-1]

    Input layout (interleaved by group, 8 rows per chunk):
      Channel ch, row r, col c:
        group = r // 8
        local_row = r % 8
        offset = (group * 256 + ch) * 128 + local_row * 16 + c
    ...
    Output layout (interleaved by group, 4-byte accumulators):
      Filter f, row r, col c:
        ...
        local_row = r % 8
        elem = local_row * 16 + c
        byte_offset = (group * 128 + f) * 512 + elem * 4

    Returns 2 * 128 * 512 = 131072 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    kernel_size = OUT_CHANNELS * TAPS_PER_FILTER  # 294912
    ...
    golden = _reference_conv_16x16_256to128(input_bytes, kernel_bytes, dtype)
