"""Standard 3x3 convolution harness (128 input -> 128 output channels, 16x16).

16x16 spatial, 8 rows packed per 128-byte chunk (2 groups).
Each output channel has 128 input-channel 3x3 kernels (1152 bytes total).
Since 1152 > 128, each filter is stored as 16 x 128-byte blocks:
  block 0:  input channels 0-7    (72 bytes + 56 padding)
  block 1:  input channels 8-15   (72 bytes + 56 padding)
  ...
  block 15: input channels 120-127 (72 bytes + 56 padding)
"""
...
TAPS_PER_FILTER = IN_CHANNELS * KERNEL_SIZE  # 1152
...
MASK_BASE_ADDR = 0x080000

# Each filter: 16 blocks x 128 bytes = 2048 bytes
CHANNELS_PER_BLOCK = 8
BLOCKS_PER_FILTER = IN_CHANNELS // CHANNELS_PER_BLOCK  # 16
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * 128  # 2048
KERNEL_TOTAL_BYTES = OUT_CHANNELS * FILTER_PADDED_BYTES  # 262144
...
def _build_kernel_data(kernel_raw: bytes) -> bytes:
    """Pack kernel into 128 filters x 16 blocks x 128 bytes.

    Input: 147456 bytes (128 filters x 128 input channels x 9 taps, contiguous).
      kernel_raw[f * 1152 + ic * 9 + tap]

    Output: 262144 bytes (128 filters x 16 blocks x 128 bytes).
...
class Conv16x16x128App(IpuApp):
    """Standard 3x3 convolution application harness (128->128 channels, 16x16).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (2 groups x 128 ch x 128 bytes = 32768 bytes).
        kernel_path: Path to kernel binary (147456 bytes = 128 x 128 x 9 taps, raw).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (2 groups x 128 channels x 128 bytes = 32768 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (147456 bytes raw -> padded to 262144 bytes)
