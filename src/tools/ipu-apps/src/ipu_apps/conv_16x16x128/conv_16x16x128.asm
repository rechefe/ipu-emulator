# Standard 3x3 convolution: 128 input -> 128 output channels, 16x16.
#
...
#
# Each output channel has 128 input-channel 3x3 kernels (full cross-channel
# mixing).  Kernel per filter: 128 * 9 = 1152 bytes, which exceeds r0's
# 128 bytes.  Solution: split into 16 input-channel groups of 8, each
# fitting in one r0 load (72 bytes + 56 padding = 128 bytes per block).
#
# Memory layout (set via CR registers):
#   cr0 = input  base  (2 groups x 128 ch x 128 bytes = 32768 bytes)
#   cr1 = kernel base  (128 filters x 16 blocks x 128 bytes = 262144 bytes)
#   cr2 = output base  (2 groups x 128 out_ch x 512 bytes = 131072 bytes)
#   cr3 = mask   base  (128 bytes: 8 mask slots x 16 bytes each)
#
# Input layout (interleaved by group, 128 channels):
#   group g, channel ch, local_row lr, col c:
#     offset = (g * 128 + ch) * 128 + lr * 16 + c
#   Group stride = 128 * 128 = 16384 bytes
#
# Output layout (interleaved by group, 32-bit accumulators):
#   group g, filter f: offset = (g * 128 + f) * 512
#
# Kernel layout (128 filters x 16 blocks x 128 bytes = 262144 bytes):
#   Filter f, block b (0..15):
#     At XMEM offset (f * 16 + b) * 128 from kernel base.
...
#   lr6  = 262144 (filter loop limit = 128 filters x 2048 bytes/filter)
#   lr7  = (unused permanent)
#   lr8  = 1     (mask_slot_1 = kc=-1)
#   lr9  = 2     (mask_slot_2 = kc=+1)
#   lr10 = channel offset (0, 128, ..., 16256, spans all 16 channel groups)
#   lr11 = 1024  (input-channel group size = 8 ch x 128 bytes)
#   lr12 = temp  (S1 address)
#   lr13 = channel loop end (lr10 + 1024)
#   lr14 = kernel memory offset (0, 128, ..., 262016)
#   lr15 = 16384 (row-group stride = 128 x 128, also total channels x 128)
#
# Structure:
#   Each filter needs 16 r0 loads (8 input channels each).
