"""Generate test data for the 16x16 128->128 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

16x16 spatial, 128 input channels, 128 output channels, 3x3 kernel.
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    kernel_size = OUT_CHANNELS * TAPS_PER_FILTER  # 147456
    ...
    golden = _reference_conv_16x16x128(input_bytes, kernel_bytes, dtype)
