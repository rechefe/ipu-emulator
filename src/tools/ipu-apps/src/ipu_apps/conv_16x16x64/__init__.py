"""Standard 3x3 convolution harness (64 input -> 64 output channels, 16x16).

16x16 spatial, 8 rows packed per 128-byte chunk (2 groups).
Each output channel has 64 input-channel 3x3 kernels (576 bytes total).
Since 576 > 128, each filter is stored as 8 x 128-byte blocks:
  block 0: input channels 0-7   (72 bytes + 56 padding)
  block 1: input channels 8-15  (72 bytes + 56 padding)
  ...
  block 7: input channels 56-63 (72 bytes + 56 padding)
"""
...
TAPS_PER_FILTER = IN_CHANNELS * KERNEL_SIZE  # 576
...
OUTPUT_BASE_ADDR = 0x020000
MASK_BASE_ADDR = 0x030000

# Each filter: 8 blocks x 128 bytes = 1024 bytes
CHANNELS_PER_BLOCK = 8
BLOCKS_PER_FILTER = IN_CHANNELS // CHANNELS_PER_BLOCK  # 8
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * 128  # 1024
KERNEL_TOTAL_BYTES = OUT_CHANNELS * FILTER_PADDED_BYTES  # 65536
...
def _build_kernel_data(kernel_raw: bytes) -> bytes:
    """Pack kernel into 64 filters x 8 blocks x 128 bytes.

    Input: 36864 bytes (64 filters x 64 input channels x 9 taps, contiguous).
      kernel_raw[f * 576 + ic * 9 + tap]

    Output: 65536 bytes (64 filters x 8 blocks x 128 bytes).
...
class Conv16x16x64App(IpuApp):
    """Standard 3x3 convolution application harness (64->64 channels, 16x16).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (2 groups x 64 ch x 128 bytes = 16384 bytes).
        kernel_path: Path to kernel binary (36864 bytes = 64 x 64 x 9 taps, raw).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (2 groups x 64 channels x 128 bytes = 16384 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (36864 bytes raw -> padded to 65536 bytes)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 2 groups x 64 output channels = 128 chunks of 512 bytes
