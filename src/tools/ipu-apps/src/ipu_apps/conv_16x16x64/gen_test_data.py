"""Generate test data for the 16x16 64->64 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

16x16 spatial, 64 input channels, 64 output channels, 3x3 kernel.
...
def _reference_conv_16x16x64(
    ...
    Output layout (interleaved by group, 4-byte accumulators):
      Filter f, row r, col c:
        ...
        local_row = r % 8
        elem = local_row * 16 + c
        byte_offset = (group * 64 + f) * 512 + elem * 4

    Returns 2 * 64 * 512 = 65536 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    kernel_size = OUT_CHANNELS * TAPS_PER_FILTER  # 36864
    ...
    golden = _reference_conv_16x16x64(input_bytes, kernel_bytes, dtype)
