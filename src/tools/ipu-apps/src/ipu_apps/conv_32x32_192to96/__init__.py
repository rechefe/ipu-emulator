"""Standard 3x3 convolution harness (192 input -> 96 output channels, 32x32).

32x32 spatial, 4 rows packed per 128-byte chunk (8 groups).
Each output channel has 192 input-channel 3x3 kernels (1728 bytes total).
Since 1728 > 128, each filter is stored as 24 x 128-byte blocks:
  block 0:  input channels 0-7     (72 bytes + 56 padding)
  block 1:  input channels 8-15    (72 bytes + 56 padding)
  ...
  block 23: input channels 184-191 (72 bytes + 56 padding)
"""
...
TAPS_PER_FILTER = IN_CHANNELS * KERNEL_SIZE  # 1728

ROWS_PER_CHUNK = 128 // COLS  # 4
NUM_GROUPS = ROWS // ROWS_PER_CHUNK  # 8

INPUT_BASE_ADDR = 0x000000
KERNEL_BASE_ADDR = 0x030000
OUTPUT_BASE_ADDR = 0x080000
MASK_BASE_ADDR = 0x0E0000

# Each filter: 24 blocks x 128 bytes = 3072 bytes
CHANNELS_PER_BLOCK = 8
BLOCKS_PER_FILTER = IN_CHANNELS // CHANNELS_PER_BLOCK  # 24
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * 128  # 3072
KERNEL_TOTAL_BYTES = OUT_CHANNELS * FILTER_PADDED_BYTES  # 294912
...
def _build_kernel_data(kernel_raw: bytes) -> bytes:
    """Pack kernel into 96 filters x 24 blocks x 128 bytes.

    Input: 165888 bytes (96 filters x 192 input channels x 9 taps, contiguous).
      kernel_raw[f * 1728 + ic * 9 + tap]

    Output: 294912 bytes (96 filters x 24 blocks x 128 bytes).
...
class Conv32x32x192to96App(IpuApp):
    """Standard 3x3 convolution application harness (192->96 channels, 32x32).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (8 groups x 192 ch x 128 bytes = 196608 bytes).
        kernel_path: Path to kernel binary (165888 bytes = 96 x 192 x 9 taps, raw).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (8 groups x 192 channels x 128 bytes = 196608 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (165888 bytes raw -> padded to 294912 bytes)
