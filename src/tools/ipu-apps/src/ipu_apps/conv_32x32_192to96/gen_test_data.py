"""Generate test data for the 32x32 192->96 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

32x32 spatial, 192 input channels, 96 output channels, 3x3 kernel.
...
def _reference_conv_32x32_192to96(
    input_bytes: bytes, kernel_bytes: bytes, dtype: DType
) -> bytes:
    """Compute the reference standard convolution output (192->96, 32x32).

    For each output filter f, output pixel (r, c):
      acc = sum over ic, dr, dc:
              kernel[f*1728 + ic*9 + dr*3 + dc] * input[ic][r+dr-1][c+dc-1]

    Input layout (interleaved by group, 4 rows per chunk):
      Channel ch, row r, col c:
        group = r // 4
        local_row = r % 4
        offset = (group * 192 + ch) * 128 + local_row * 32 + c
    ...
    Output layout (interleaved by group, 4-byte accumulators):
      Filter f, row r, col c:
        ...
        local_row = r % 4
        elem = local_row * 32 + c
        byte_offset = (group * 96 + f) * 512 + elem * 4

    Returns 8 * 96 * 512 = 393216 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROWS * COLS * IN_CHANNELS  # 196608
    kernel_size = OUT_CHANNELS * TAPS_PER_FILTER  # 165888
    ...
    golden = _reference_conv_32x32_192to96(input_bytes, kernel_bytes, dtype)
