"""Standard 3x3 convolution harness (16 input -> 16 output channels, 32x32).

32x32 spatial, 4 rows packed per 128-byte chunk (8 groups).
Each output channel has 16 input-channel 3x3 kernels (144 bytes total).
Since 144 > 128, each filter is stored as 2 x 128-byte blocks:
...
MASK_BASE_ADDR = 0x020000
OUTPUT_BASE_ADDR = 0x030000

# Each filter: 2 blocks x 128 bytes = 256 bytes
BLOCKS_PER_FILTER = 2
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * 128  # 256
...
def _build_kernel_data(kernel_raw: bytes) -> bytes:
    """Pack kernel into 16 filters x 2 blocks x 128 bytes.

    Input: 2304 bytes (16 filters x 16 input channels x 9 taps, contiguous).
      kernel_raw[f * 144 + ic * 9 + tap]

    Output: 4096 bytes (16 filters x 2 blocks x 128 bytes).
      Block 0: input channels 0-7 (72 bytes + 56 padding)
      Block 1: input channels 8-15 (72 bytes + 56 padding)
    """
    kernel_padded = bytearray(KERNEL_TOTAL_BYTES)
    taps_per_half = 8 * KERNEL_SIZE  # 72 bytes per block
...
class Conv32x32x16App(IpuApp):
    """Standard 3x3 convolution application harness (16->16 channels, 32x32).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (8 groups x 16 ch x 128 bytes = 16384 bytes).
        kernel_path: Path to kernel binary (2304 bytes = 16 x 16 x 9 taps, raw).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (8 groups x 16 channels x 128 bytes = 16384 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (2304 bytes raw -> padded to 4096 bytes)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 8 groups x 16 output channels = 128 chunks of 512 bytes
