# Standard 3x3 convolution: 16 input -> 16 output channels, 32x32.
#
...
#
# Each output channel has 16 input-channel 3x3 kernels (full cross-channel
# mixing).  Kernel per filter: 16 * 9 = 144 bytes, which exceeds r0's
# 128 bytes.  Solution: split into 2 input-channel groups of 8, each
# fitting in one r0 load (72 bytes + 56 padding = 128 bytes per block).
#
# Memory layout (set via CR registers):
#   cr0 = input  base  (8 groups x 16 ch x 128 bytes = 16384 bytes)
#   cr1 = kernel base  (16 filters x 2 blocks x 128 bytes = 4096 bytes)
#   cr2 = output base  (8 groups x 16 out_ch x 512 bytes = 65536 bytes)
#   cr3 = mask   base  (128 bytes: 8 mask slots x 16 bytes each)
#
# Input layout (interleaved by group, 16 channels):
#   group g, channel ch, local_row lr, col c:
#     offset = (g * 16 + ch) * 128 + lr * 32 + c
#   Group stride = 16 * 128 = 2048 bytes
#
# Output layout (interleaved by group, 32-bit accumulators):
#   group g, filter f: offset = (g * 16 + f) * 512
#
# Kernel layout (16 filters x 2 blocks x 128 bytes = 4096 bytes):
#   Filter f, block b (0..1):
#     At XMEM offset (f * 2 + b) * 128 from kernel base.
#     Block 0: input channels 0-7 (72 bytes data + 56 padding)
#     Block 1: input channels 8-15 (72 bytes data + 56 padding)
...
#   lr6  = 4096  (filter loop limit = 16 filters x 256 bytes/filter)
#   lr7  = row-group base offset (g * 2048)
#   lr8  = 1     (mask_slot_1 = kc=-1)
#   lr9  = 2     (mask_slot_2 = kc=+1)
#   lr10 = channel offset (0, 128, ..., 1920, spans both channel groups)
...
#   lr15 = 2048  (row-group stride = 16 x 128, also total channels x 128)
#
# Structure:
#   Each filter needs 2 r0 loads (8 input channels each).
#   After the first 8 channels, reload r0 and continue accumulating.
#   For each row-group section (top, middle, bottom):
#     Filter loop (16 filters): reset_acc, process 2 channel groups, store.
...
g0_ch_loop:
    ...
    # Check if second channel group needed
    incr                lr14 128;;

    blt                 lr10 lr15 g0_reload;;

    # Both groups done -- store output and advance filter
    str_acc_reg         lr2 cr2;;
