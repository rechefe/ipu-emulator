"""Standard 3x3 convolution harness (64 input -> 64 output channels, 32x32).
...
class Conv32x32x64App(IpuApp):
    """Standard 3x3 convolution application harness (64->64 channels, 32x32).

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input binary (8 groups x 64 ch x 128 bytes = 65536 bytes).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (8 groups x 64 channels x 128 bytes = 65536 bytes)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 8 groups x 64 output channels = 512 chunks of 512 bytes
