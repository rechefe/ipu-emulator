"""Generate test data for the 32x32 64->64 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

32x32 spatial, 64 input channels, 64 output channels, 3x3 kernel.
...
def _reference_conv_32x32x64(
    input_bytes: bytes, kernel_bytes: bytes, dtype: DType
) -> bytes:
    """Compute the reference standard convolution output (64->64, 32x32).
    ...
    Input layout (interleaved by group, 4 rows per chunk):
      Channel ch, row r, col c:
        group = r // 4
        local_row = r % 4
        offset = (group * 64 + ch) * 128 + local_row * 32 + c
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    golden = _reference_conv_32x32x64(input_bytes, kernel_bytes, dtype)
