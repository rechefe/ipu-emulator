# Standard 3x3 convolution: 96 input -> 96 output channels, 32x32.
#
...
#
# Each output channel has 96 input-channel 3x3 kernels (full cross-channel
# mixing).  Kernel per filter: 96 * 9 = 864 bytes, which exceeds r0's
# 128 bytes.  Solution: split into 12 input-channel groups of 8, each
# fitting in one r0 load (72 bytes + 56 padding = 128 bytes per block).
#
# Memory layout (set via CR registers):
#   cr0 = input  base  (8 groups x 96 ch x 128 bytes = 98304 bytes)
#   cr1 = kernel base  (96 filters x 12 blocks x 128 bytes = 147456 bytes)
#   cr2 = output base  (8 groups x 96 out_ch x 512 bytes = 393216 bytes)
#   cr3 = mask   base  (128 bytes: 8 mask slots x 16 bytes each)
#
# Input layout (interleaved by group, 96 channels):
#   group g, channel ch, local_row lr, col c:
#     offset = (g * 96 + ch) * 128 + lr * 32 + c
#   Group stride = 96 * 128 = 12288 bytes
#
# Output layout (interleaved by group, 32-bit accumulators):
#   group g, filter f: offset = (g * 96 + f) * 512
#
# Kernel layout (96 filters x 12 blocks x 128 bytes = 147456 bytes):
#   Filter f, block b (0..11):
#     At XMEM offset (f * 12 + b) * 128 from kernel base.
...
#   lr6  = 147456 (filter loop limit = 96 filters x 1536 bytes/filter)
#   lr7  = row-group base offset (g * 12288)
#   lr8  = 1     (mask_slot_1 = kc=-1)
#   lr9  = 2     (mask_slot_2 = kc=+1)
#   lr10 = channel offset (0, 128, ..., 12160, spans all 12 channel groups)
...
#   lr14 = kernel memory offset (0, 128, ..., 147328)
#   lr15 = 12288 (row-group stride = 96 x 128, also total channels x 128)
#
# Structure:
#   Each filter needs 12 r0 loads (8 input channels each).
#   After the first 8 channels, reload r0 and continue accumulating.
#   For each row-group section (top, middle, bottom):
#     Filter loop (96 filters): reset_acc, process 12 channel groups, store.
...
# ===========================================================================
    ...
    # lr6 = 96 filters x 1536 bytes/filter = 147456 = 12 x lr15
    add                 lr6 lr15 lr15;;
...
ch_loop:
    ...
    # Middle loop limit: 7 * 12288 = 86016
    set                 lr13 21504;;
