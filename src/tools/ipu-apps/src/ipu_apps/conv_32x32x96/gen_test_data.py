"""Generate test data for the 32x32 96->96 channel convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

32x32 spatial, 96 input channels, 96 output channels, 3x3 kernel.
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROWS * COLS * IN_CHANNELS  # 98304
    kernel_size = OUT_CHANNELS * TAPS_PER_FILTER  # 82944
    ...
    golden = _reference_conv_32x32x96(input_bytes, kernel_bytes, dtype)
