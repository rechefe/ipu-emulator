"""Standard 3x3 convolution harness (64 input -> 64 output channels, 8x8).
...
Kernel per filter: 64 * 9 = 576 bytes -> 8 r0 blocks of 128 bytes each.
"""
...
IN_CHANNELS = 64
OUT_CHANNELS = 64
NUM_PAIRS = OUT_CHANNELS // 2  # 32

INPUT_BASE_ADDR = 0x00000
KERNEL_BASE_ADDR = 0x01000
MASK_BASE_ADDR = 0x11000
OUTPUT_BASE_ADDR = 0x11200

# Input: 32 packed chunks x 128 bytes = 4096 bytes
NUM_CHUNKS = IN_CHANNELS // 2  # 32

# Kernel: 8 blocks per filter, 128 bytes each
BLOCKS_PER_FILTER = 8
CHANNELS_PER_BLOCK = 8
TAPS_PER_CHANNEL = 9
FILTER_BLOCK_BYTES = 128
FILTER_PADDED_BYTES = BLOCKS_PER_FILTER * FILTER_BLOCK_BYTES  # 1024
...
class Conv8x8_64to64App(IpuApp):
    """Standard 3x3 convolution application harness (64->64 channels, 8x8).
    ...
    Args:
        ...
        input_path:  Path to input binary (4096 bytes: 32 chunks x 128 bytes).
        kernel_path: Path to kernel binary (65536 bytes: 64 filters x 1024 bytes).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input (32 chunks x 128 bytes = 4096 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (64 filters x 8 blocks x 128 bytes = 65536 bytes)
