"""Generate test data for the standard 64->64 channel convolution app (8x8).

Input layout (packed pairs, 2 channels per 128-byte chunk):
  chunk j (j=0..31): channels 2j and 2j+1
...
Kernel layout per filter f:
  Block b (offset f*1024 + b*128): channels b*8..(b+1)*8-1
...
FILTER_PADDED = BLOCKS_PER_FILTER * FILTER_BLOCK_BYTES  # 1024
...
def _reference_conv_8x8_64to64(
    input_bytes: bytes, kernel_data: list[bytes], dtype: DType
) -> bytes:
    """Compute the reference standard convolution output (64->64, 8x8).

    Input is packed: 2 channels per 128-byte chunk.
    Output: 64 filters x 64 elements x 4-byte accumulators = 16384 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROWS * COLS * IN_CHANNELS  # 4096
    ...
    # Pack kernel: 8 blocks of 128 bytes per filter
    ...
    golden = _reference_conv_8x8_64to64(
