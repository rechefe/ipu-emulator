...
g0_ch_loop:
    ...
    DEC                 {{ lr_kern_idx }} 1;;                        # tap 1 INC brings it to 0 (no bias byte)
...
ch_loop:
    ...
    #     lr_kern_idx := -1 (tap 1's INC brings it to 0).  lr_off_zero stays = E0.
    add                 {{ lr_walk }} {{ lr_walk }} {{ cr_cols }};
    add                 {{ lr_write }} {{ lr_zero }} {{ cr_chunk_bytes }};
    DEC                 {{ lr_kern_idx }} 1;;                        # tap 1 INC brings it to 0 (no bias byte)
...
gN_ch_loop:
    ...
    # Cy 4: lr_walk = cols - 383.  lr_write := 128.  lr_kern_idx := -1.
    add                 {{ lr_walk }} {{ lr_walk }} {{ cr_cols }};
    add                 {{ lr_write }} {{ lr_zero }} {{ cr_chunk_bytes }};
    DEC                 {{ lr_kern_idx }} 1;;                        # tap 1 INC brings it to 0 (no bias byte)
