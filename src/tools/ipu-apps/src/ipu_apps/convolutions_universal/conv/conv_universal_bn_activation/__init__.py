...
# The input region does NOT start at row 0: the top-border (g0) section
# prefetches the kr=-1 chunk as ``E0 - cr6`` which, at chunk 0, is one group
# stride BELOW the input base. That value is deliberately garbage (mask slot 3
# discards it), but it still has to be a *legal* address. Under the old 21-bit
# LR/CR mask the negative offset silently wrapped into the 2 MB space; mb/195
# widens that mask to 32 bits and range-checks the row, so the underflow now
# faults.
#
# The fix cannot go in the base alone: the asm computes the kr=-1 address as an
# LR *offset* (``lr_off_zero - cr6``) that is added to ``cr_input_base`` only
# inside the load, so the offset itself goes negative and wraps to a huge
# unsigned value before the base is ever applied. So both sides move together:
# the input data sits one group stride above ``cr_input_base``, and the asm
# seeds ``lr_chunk_base`` to that same stride (``cr6``) instead of 0. The
# prefetch's worst case is then exactly offset 0 -- in bounds, and still the
# don't-care value mask slot 3 discards.
...
def _pack_conv_weights_fpb28(
    ...
    # The layout is ELEMENT-identical in both modes -- element index e always
    # means the same weight -- only the byte scale differs (1 B/element narrow,
    # 4 B/element in wide-vector debug). MULT.VE/MULT.EE index ra by lane in
    # wide mode and by byte in narrow mode, so widening here is what keeps the
    # single .asm correct in both.
    ...
    for f in range(out_ch):
        for sb in range(super_blocks_per_filter):
            ...
            put(sb_base, bias_bytes[f])       # element 0 = filter bias
            for s in range(FPB):
                ic = sb * FPB + s
                if ic >= in_ch:
                    break
...
class ConvUniversalBnActivationApp(IpuApp):
    ...
    def _pack_kernel(self) -> bytes:
        ...
        return _pack_conv_weights_fpb28(
            w_reordered, self.dtype, self._bias_array, self._element_width
        )
