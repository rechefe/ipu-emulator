...
Like the conv_universal benchmark, but the reference is conv + per-filter
folded bias + ReLU (matching the app's ACTIVATE relu pipeline), compared
against a real PyTorch reference with a tolerance (FP32 wide-vector mode).

Runs each config through the emulator, and reports cycles and MULT-slot
utilization read directly from ``state.stats`` (the emulator's live
per-cycle occupancy counter).
...
# (height, width, in_ch, out_ch) — covers shapes, channel counts, partial vs
# full last super-blocks (FPB=28), and small/large spatial sizes.
CONFIGS = [
    (16, 16, 28, 4),     # exactly one full super-block
    (16, 16, 16, 4),     # partial last block
    (32, 32, 28, 8),     # multi-chunk
    ...
    (16, 16, 56, 4),     # two super-blocks (bias accumulated once)
]
