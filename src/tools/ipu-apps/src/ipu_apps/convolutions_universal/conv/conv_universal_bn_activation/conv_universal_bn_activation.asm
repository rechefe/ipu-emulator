...
#   The 512-ELEMENT cyclic register holds three vertically-neighboring
#   128-element chunks of one input channel.  The ring is 512 elements in both
#   narrow and wide-vector debug mode (only its byte size differs), so every
#   offset below is an element index and needs no per-mode scaling.
#   Slot bases (mod 512):
...
#   tap 1: lr5 = lr_read - cr12  (= lr_read - 128 = lr_read + 384 mod 512) [elements]
...
#   XMEM-space (ROW numbers, scaled by the active mode's row size):
#     cr2/cr3/cr5/cr10 = output/mask/kernel/input base ROWS
#     cr6 = in-channel group stride (rows), cr7 = 28 (channel group, rows)
#     cr8 = total kernel rows, cr11 = chunk-loop limit (rows)
#     cr1 = 1 row (read-only const; the row-space chunk stride)
#   R_CYCLIC-space (ELEMENT indices; ring is 512 elements in BOTH modes):
#     cr12 = 128 (slot stride), cr13 = 2 rows (kernel super-block, XMEM)
#     cr9  = 384 (ring advance), cr14 = 384 - 2*cols - 2 (end-of-9 walk step)
#
# LR allocation:
#   lr0  = 0
...
#   lr4  = lr_read
#   lr5  = lr_write
#   lr6  = kernel byte index (ELEMENT index within the mult reg, not XMEM)
#   lr7  = output pointer
#   lr8  = chunk base ROW
#   lr10 = channel ROW counter (for blt loop term)
#   lr11 = clamp limit
#   lr12 = kernel super-block offset (ROWS)
#   lr13 = scratch
#   lr14 = scratch
#   (lr15 free)
#
