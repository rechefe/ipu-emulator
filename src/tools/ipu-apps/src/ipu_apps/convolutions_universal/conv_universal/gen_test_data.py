"""Generate test data for the universal standard convolution app.
...
Usage::
    ...
    python -m ipu_apps.convolutions_universal.conv_universal.gen_test_data \\
...
def reference_conv(
    ...
    """Compute the reference standard 3x3 convolution output.

    For each output filter f, output pixel (r, c):
      acc = sum over ic, dr, dc:
              kernel[f * in_ch * 9 + ic * 9 + dr * 3 + dc]
              * input[ic][r+dr-1][c+dc-1]
    ...
    Input layout (interleaved by chunk group):
      Channel ch, row r, col c:
        ...
        offset = (chunk * in_channels + ch) * 128 + local_row * cols + c

    Kernel layout: kernel[f * in_channels * 9 + ic * 9 + dr * 3 + dc]
    ...
    Output layout (interleaved, 4-byte accumulators):
      Filter f, row r, col c:
        ...
        byte_offset = (chunk * out_channels + f) * 512 + elem * 4
    """
    rows_per_chunk = 128 // cols
    ...
    output = bytearray(num_chunks * out_channels * 512)
                ...
                for ic in range(in_channels):
                    for dr in range(3):
                        for dc in range(3):
                            ir = r + dr - 1
                            ic_ = c + dc - 1
                            if 0 <= ir < rows and 0 <= ic_ < cols:
                                ki = f * taps_per_filter + ic * 9 + dr * 3 + dc
                                a = kernel_bytes[ki]
                                ig = ir // rows_per_chunk
                                ilr = ir % rows_per_chunk
                                in_idx = (ig * in_channels + ic) * 128 + ilr * cols + ic_
...
def generate_for_dtype(
    ...
    kernel_size = out_channels * in_channels * 9
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate test data for universal standard convolution.",
    )
