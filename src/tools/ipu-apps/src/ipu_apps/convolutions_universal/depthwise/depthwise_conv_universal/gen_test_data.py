...
Usage::
    ...
    python -m ipu_apps.convolutions_universal.depthwise.depthwise_conv_universal.gen_test_data \\
...
def reference_depthwise_conv(
    ...
    Output layout (INT8): interleaved, 1 byte per element.
      Channel ch, row r, col c:
        chunk = r // rows_per_chunk
        local_row = r % rows_per_chunk
        elem = local_row * cols + c
        byte_offset = (chunk * channels + ch) * 128 + elem
    """
    import struct
    rows_per_chunk = 128 // cols
        ...
        for ch in range(channels):
            for r in range(rows):
                for c in range(cols):
                    ...
                    out_elem = olr * cols + c
                    out_idx = (og * channels + ch) * 128 + out_elem
        ...
        for ch in range(channels):
            for r in range(rows):
                for c in range(cols):
                    ...
                    struct.pack_into("<f", output, out_idx, acc)
