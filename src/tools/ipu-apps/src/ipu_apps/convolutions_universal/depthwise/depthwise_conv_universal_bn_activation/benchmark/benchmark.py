...
Like the depthwise_conv_universal benchmark, but the reference is depthwise
conv + per-channel folded bias + ReLU (matching the app's bias-seed ->
ACTIVATE relu pipeline), compared against a real PyTorch reference with a
tolerance (FP32 wide-vector mode).
...
# (height, width, channels) — spatial sizes + FPB=25 boundary cases.
CONFIGS = [
