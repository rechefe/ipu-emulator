# Universal Depthwise 3x3 Convolution
#
# A single binary that handles ANY valid depthwise convolution configuration.
...
# Works for ALL cols values (16, 32, 64, 128).
#
...
#   slot 3: bottom row         -> zero last spatial row in chunk (last chunk, kr=+1, kc=0)
#   slot 4: left + bottom      -> union of slot 1 and slot 3 (last chunk, kr=+1, kc=-1)
#   slot 5: right + bottom     -> union of slot 2 and slot 3 (last chunk, kr=+1, kc=+1)
#
...
#   lr6  = kernel byte index (0..71 per kernel group)
#   lr7  = output pointer
#   lr8  = chunk base address (chunk_index * group_stride)
#   lr9  = chunk counter
#   lr10 = channel offset within chunk (0, 128, ..., group_stride-128)
#   lr11 = channel group limit (lr10 + 1024)
#   lr12 = kernel memory offset (0, 128, ..., (num_groups-1)*128)
...
ch_loop:
    ...
    # Note: lr15 trashed here (=256), but we restore it after the loop
...
# to zero the last spatial row's contribution from stale S2 data.
# ===========================================================================
