...
# and is unaffected by this migration. This app has NO spatial taps (1x1
...
# increases.
#
# cr12 (=128) was overloaded pre-migration between the r_cyclic-adjacent
# walking step (now handled by lr6/lr8/lr9's ELEMENT-space roles, unchanged)
# and several XMEM-space byte-128 strides (output stride, kernel-row advance,
# output pre-offset). Those XMEM roles now use CR1 (read-only 1: one XMEM
# chunk == one row); cr12 keeps its one remaining role, the fixed_idx step
# (128 lanes) for Half B, which is lane/element space and mode-blind.
#
...
# values (128/256/384, unchanged). But they used to ALSO be added straight
# into lr0/lr13/lr14 to form the NEXT XMEM read address -- that byte-space use
# does not survive row addressing (128 bytes == 1 row, not 128 rows). Those
# one-shot "+2 rows" / "+3 rows" computations (base + lr8 / base + lr9) are
# now built from two/three chained `add ... cr1` words instead (ADD's src_a
...
# iteration) becomes `lr14 += cr1` directly.
#
