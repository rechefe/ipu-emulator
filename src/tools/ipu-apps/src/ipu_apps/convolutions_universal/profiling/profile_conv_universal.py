"""Profile memory access patterns for conv_universal (3x3, flexible spatial/channels).
...
Run from the repo root:
    PYTHONPATH=src/tools/ipu-emu-py/src:src/tools/ipu-common/src:src/tools/ipu-apps/src:src/tools/ipu-as-py/src \
        python -m ipu_apps.convolutions_universal.profiling.profile_conv_universal
"""
...
def main() -> None:
    ...
    for rows, cols, ic, oc in CONFIGS:
        ...
        kernel_bytes = rng.randint(0, 256, oc * ic * 9, dtype=np.uint8).tobytes()
        ...
        try:
            ...
            result = run_profile_safe(app, CR_NAMES, max_cycles=10_000_000)
