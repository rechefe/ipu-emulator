"""Debug runner for the depthwise convolution app.

Usage::

    bazel run //src/tools/ipu-apps:depthwise_conv -- --dtype INT8
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run depthwise conv with debug CLI")
    ...
    inst_path = Path(os.environ["DCONV_INST_BIN"])
    data_dir = Path(os.environ["DCONV_DATA_DIR"])
