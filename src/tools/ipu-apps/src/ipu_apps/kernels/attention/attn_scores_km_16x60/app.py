...
This is the L5 variant of ``attn_scores_km_256x36``; the mapping is unchanged and
...
# kernel's exact-match shape domain (n_tok=16, d=60) and for the optional
# `head` query parameter: it selects which of the 4 heads already present in
# the input file to compute, not a shape the query routes on -- every head is
# equally supported by the same kernel, so a query that never mentions `head`
# still gets a correct verdict (head 0). `build` hands it to the constructor,
# which keeps its own range check for direct construction.

SPEC = attention_spec(
    AttnScoresKM16x60App,
    op="attn_scores_km",
    n_tok=N_TOK,
    d=D,
    heads=N_HEADS,
