...
Usage::
    ...
    from ipu_apps.kernels.attention.attn_scores_km_256x36.app import AttnScoresKM256x36App
"""
...
# kernel's exact-match shape domain (n_tok=256, d=36) and for the optional
# `head` query parameter: it picks which of the 4 heads already in the input
# file to compute, not a shape dimension, so a query that never mentions it
# still routes (head 0). `build` hands it to the constructor, which keeps its
# own range check for direct construction.

SPEC = attention_spec(
    AttnScoresKM256x36App,
    op="attn_scores_km",
    n_tok=N_TOK,
    d=D,
    heads=N_HEADS,
