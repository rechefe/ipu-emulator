...
This is the L4 variant of ``attn_scores_km_256x36``, and it keeps that kernel's
...
from ipu_apps.kernels.attention.app import attention_spec
...
# Producers emit full rows; only the consumer crops (see
# docs/content/kernels/mobilevit.md).
...
# Declared beside the kernel so the registry needs no central list.
# `supports` (built by attention_spec) is the single source of truth for this
# kernel's exact-match shape domain (n_tok=64, d=48) and for the optional
# `head` query parameter: it selects which of the N_HEAD heads already in the
# input file to compute, not a shape the query routes on, so a query that
# never mentions it still routes (head 0). `build` hands it to the
# constructor, which keeps its own range check for direct construction.

SPEC = attention_spec(
    AttnScoresKM64x48App,
    op="attn_scores_km",
    n_tok=N,
    d=D,
    heads=N_HEAD,
    explain=(
        f"n_tok == {N} and d == {D} exactly: the L4 key-major kQT scores "
        f"kernel, one selected head across all {P} streams ({N_BLOCK} blocks "
        f"in the input file)."
    ),
    bundle=lambda **params: ShapeBundle.of(
        query=(P, params["n_tok"], params["d"]),
        key=(P, params["n_tok"], params["d"]),
    ).with_shapes(derived={"output": (P, params["n_tok"], params["n_tok"])}),
)
