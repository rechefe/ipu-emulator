...
This is the L5 variant of ``attn_v_256x36``; the mapping (query-major P, lanes =
...
# kernel's exact-match shape domain (n_tok=16, d=60). The relocatable
# pbase_row/vbase_row/obase_row kwargs (and the p_path=None/v_path=None
# chained-kernel skip) are placement/staging concerns, not shape the registry
# routes on, so `build` leaves them at their constructor defaults -- a caller
# that needs a non-default layout still constructs the app directly.
...
SPEC = attention_spec(
    ...
    d=D,
    explain=(
        f"n_tok == {N_TOK} and d == {D} exactly: the L5 attn@V AGG kernel "
        f"(query-major P), all {N_HEAD} heads in one invocation."
    ),
    bundle=lambda **params: ShapeBundle.of(
        attn=(params["n_tok"], params["n_tok"]), value=(params["n_tok"], params["d"])
    ).with_shapes(derived={"output": (params["n_tok"], params["d"])}),
)
