...
Usage::
    ...
    from ipu_apps.kernels.attention.attn_v_256x36.app import AttnV256x36App
"""
...
# kernel's exact-match shape domain (n_tok=256, d=36).
...
SPEC = attention_spec(
    ...
    d=D,
    explain=(
        f"n_tok == {N_TOK} and d == {D} exactly: the attn@V AGG kernel "
        f"(query-major P), all {N_HEAD} heads in one invocation, 256 tokens "
        f"spanning two 128-lane key chunks."
    ),
