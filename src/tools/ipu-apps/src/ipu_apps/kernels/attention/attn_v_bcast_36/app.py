...
Output (FP32 R_ACC, one row per group — transformer-matmul convention):
  O[i, t] at OBASE + chan*2 + g rows, lane local,  i = g*128 + local
  i.e. channel (h*36 + t) occupies 2 rows (2 groups of 128 FP32 lanes).

Usage::

    from ipu_apps.kernels.attention.attn_v_bcast_36.app import AttnVBcast36App
"""
...
# Unlike the other three attention ops, attn_v_bcast is indexed by `d`
# (head_dim): its three sibling apps are named and distinguished purely by
# head_dim (36/48/60), and N_TOK is a fixed module constant that is not part of
# the dirname or a caller-visible constructor parameter, so `n_tok` is absent
# from `requires` for this op. A query that does carry `n_tok` must match
# N_TOK (see the family app.py).
