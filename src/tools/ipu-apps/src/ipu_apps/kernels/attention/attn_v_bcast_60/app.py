...
This is the L5 variant of ``attn_v_bcast_36``; the MAPPING is unchanged -- lanes
...
# Like its siblings, attn_v_bcast is indexed by `d` (head_dim) -- N_TOK is a
# fixed module constant, not a caller-visible parameter, so it is absent from
# `requires`; a query that does carry `n_tok` must match it (see the family
# app.py).
...
SPEC = attention_spec(
    ...
    d=D,
    explain=(
        f"d == {D} exactly: the L5 attn@V broadcast kernel (key-major P, "
