...
This is the L4 variant of ``qk_scores_256x36``.  The mapping is carried over
...
# kernel's exact-match domain (n_tok=64, d=48). This app has no
# k_base_row-style hardware invariant to fold or exclude -- its constructor
# takes no extra kwargs.
...
SPEC = attention_spec(
    ...
    d=D,
    explain=(
        f"n_tok == {N} and d == {D} exactly: the L4 query-major QKT scores "
        f"kernel, all {N_BLOCK} (stream, head) blocks in one invocation."
    ),
    bundle=lambda **params: ShapeBundle.of(
        query=(N_BLOCK, params["n_tok"], params["d"]),
        key=(N_BLOCK, params["n_tok"], params["d"]),
    ).with_shapes(derived={"output": (N_BLOCK, params["n_tok"], params["n_tok"])}),
)
