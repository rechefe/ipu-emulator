...
Usage (normally through the registry: ``create_harness("conv_first_layer",
params=..., bindings=...)``)::

    from ipu_apps.kernels.convolutions.conv_first_layer.app import ConvFirstLayerApp

    app = ConvFirstLayerApp(
        inst_path="conv_first_layer.bin",
        input_path="input.bin",   # raw [3, 256, 256] float32
        kernel_path="kernel.bin", # raw [16, 3, 3, 3] float32 (or kernel=array)
        bias_path="bias.bin",     # raw [16] float32, folded BN bias (or bias=array)
...
def _supports(q):
    ...
    if not q.apply_relu:
        return "this kernel always applies ReLU"
    if not q.has_bias:
        return "this kernel requires bias (folded)"
