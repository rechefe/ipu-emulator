...
"conv_universal_bn_activation", params=..., bindings=...)``)::
    ...
    from ipu_apps.kernels.convolutions.conv_universal_bn_activation.app import (
    ...
    app = ConvUniversalBnActivationApp(
        ...
        bias_path="bias.bin",        # raw [out_ch] float32, folded BN bias (or bias=array)
...
# Same domain as conv_universal except this twin REQUIRES apply_relu=True (it
# unconditionally applies ReLU); bias is optional (zeros when absent).
...
SPEC = universal_spec(
    ...
    explain=lambda q: (
        ...
        f"x{q.width} pads internally to {'x'.join(map(str, _padded(q)))}."
    ),
