...
"depthwise_conv_stride2_128", params=..., bindings=...)``)::
    ...
    app = DepthwiseConvStride2_128App(
        inst_path="depthwise_conv_stride2_128.bin",  # stage 2, assembled
...
class DepthwiseConvStride2_128App(IpuApp):
    ...
    Args:
        inst_path:    Stage 2, the assembled ``depthwise_conv_stride2_128.asm``.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # inst_path is stage 2 (this kernel's own .asm, assembled); stage 1 is
        # assembled from depthwise_conv_universal's .asm in run().
    ...
    def run(self, *, max_cycles: int = 2_000_000, **kwargs) -> tuple["IpuState", int]:
        with tempfile.TemporaryDirectory(prefix="depthwise_stride2_128_") as tmp:
            stage1_bin_path = Path(tmp) / "stage1.bin"
...
def _supports(q):
    ...
    if q.width != 128:
        return (f"handles only width=128 (see depthwise_conv_stride2_narrow for cols in "
                f"16/32/64); got {q.width}")
    if q.height < 4 or q.height % 4 != 0:
        return f"height must be a multiple of 4 (>= 4) so out_rows/2 pairs evenly; got {q.height}"
    return None
...
SPEC = universal_spec(
    ...
    supports=_supports,
    build=lambda q: {"rows": q.height, "channels": q.in_channels},
    explain=lambda q: (
