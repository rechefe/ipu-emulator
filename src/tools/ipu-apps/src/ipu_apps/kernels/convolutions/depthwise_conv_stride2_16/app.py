...
``depthwise_conv_universal``):
  ...
  Stage 2 (this kernel's own ``depthwise_conv_stride2_16.asm`` -- fixed
  shape, no Jinja templating needed since nothing varies but ``channels``):
  one channel's TRUE output is exactly 8x8 = 64 elements -- HALF of one
...
"depthwise_conv_stride2_16", params=..., bindings=...)``)::
    ...
    app = DepthwiseConvStride2_16App(
        inst_path="depthwise_conv_stride2_16.bin",  # stage 2, assembled
...
class DepthwiseConvStride2_16App(IpuApp):
    ...
    def run(self, *, max_cycles: int = 2_000_000, **kwargs) -> tuple["IpuState", int]:
        with tempfile.TemporaryDirectory(prefix="depthwise_stride2_16_") as tmp:
...
def _supports(q):
    ...
    if q.in_channels % 2 != 0:
        return f"channels must be even (two channels pack per output chunk); got {q.in_channels}"
    return None
...
SPEC = universal_spec(
    ...
    supports=_supports,
    build=lambda q: {"channels": q.in_channels},
    explain=lambda q: (
