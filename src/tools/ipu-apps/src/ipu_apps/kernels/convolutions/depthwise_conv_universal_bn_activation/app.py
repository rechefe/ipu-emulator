...
"depthwise_conv_universal_bn_activation", params=..., bindings=...)``)::

    from ipu_apps.kernels.convolutions.depthwise_conv_universal_bn_activation.app import (
    ...
    app = DepthwiseConvUniversalBnActivationApp(
        ...
        bias_path="bias.bin",         # raw [channels] float32 (or bias=array; defaults zeros)
...
class DepthwiseConvUniversalBnActivationApp(IpuApp):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._bias_array = read_bias(bias, getattr(self, "bias_path", None), channels)
...
# groups == in_channels (depthwise), folded bias + ReLU.
...
def _supports(q):
    ...
    if q.out_channels != q.in_channels:
        return (f"depthwise requires out_channels==in_channels; got {q.out_channels} "
                f"vs {q.in_channels}")
    if not q.apply_relu:
        return "this kernel always applies ReLU; see depthwise_conv_universal for the plain twin"
    if not q.has_bias:
        return "this kernel requires bias (folded); see depthwise_conv_universal for the bias-free twin"
    if q.width > 128:
        return (
...
SPEC = universal_spec(
    ...
    supports=_supports,
    build=lambda q: {"height": q.height, "width": q.width, "channels": q.in_channels},
    explain=lambda q: (
        f"kernel_size=3, groups=in_channels (depthwise), stride=1, padding=1, "
        f"bias+ReLU: the universal depthwise 3x3 conv kernel (FP32). "
        f"{q.height}x{q.width} pads internally to {'x'.join(map(str, _padded(q)))}."
    ),
