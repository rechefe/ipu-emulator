...
class PointwiseConvUnifiedApp(IpuApp):
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # ACTIVATE.QUANTIZE reads its active-lane count from the named
        ...
        # With wide_vector_quantize_output=False (SPEC.execution), ACTIVATE
        # writes identity-activated FP32 into post_aaq_reg with no clamp --
        # this is NOT a quantize step in this mode; the name comes
        # from the INT8 quantize path.
...
def _supports(q):
    ...
    if q.apply_relu:
        return "apply_relu=True has no matching app here; see pointwise_conv_unified_bn_activation"
    if bad := bias_requires_relu(q):
        return bad
    if q.has_bias:
        return "bias is not supported by this kernel; see pointwise_conv_unified_bn_activation"
    return None
...
SPEC = universal_spec(
    ...
    explain=lambda q: (
        ...
        f"{'x'.join(map(str, pointwise_pad_shape(q.height, q.width)))} (no mask "
        "care needed -- a 1x1 conv has no spatial neighbourhood for a padded "
        "lane to leak into)."
    ),
