# Unified Pointwise (1x1) Convolution
#
...
# ROW numbers, not byte addresses. LDR_CYCLIC_MULT_REG's `index` (3rd operand,
# selecting the R_CYCLIC write slot) and MULT.RC.VE's `rc_idx` operand stay
# ELEMENT-indexed (0/128/256/384) -- the ring is 512 elements in both modes
# and is unaffected by row addressing. This app has NO spatial taps (1x1
# conv), so unlike the 3x3 conv/depthwise apps there is no kr=-1 prefetch and
# no negative-offset guard band needed -- lr0 starts at row 0 and only ever
# increases.
#
# cr12 (=128) is NOT used for XMEM strides. The r_cyclic-adjacent walking
# step is handled by lr6/lr8/lr9's ELEMENT-space roles, and the XMEM-space
# strides (output stride, kernel-row advance, output pre-offset) use CR1
# (read-only 1: one XMEM chunk == one row); cr12's only role is the fixed_idx
# step (128 lanes) for Half B, which is lane/element space and mode-blind.
#
# lr6/lr8/lr9 are also overloaded: as the 3rd (index) operand to
# LDR_CYCLIC_MULT_REG and the rc_idx operand to MULT.RC.VE they are ELEMENT
# values (128/256/384). They are NOT added into lr0/lr13/lr14 to form the
# NEXT XMEM read address -- in row addressing 128 elements == 1 row, not 128
# rows. The one-shot "+2 rows" / "+3 rows" computations are built from
# two/three chained `add ... cr1` words instead (ADD's src_a
# reads snapshot, so a same-word double-add into the same destination isn't
# possible -- see the row-addressing recipe's depthwise cr13 precedent). The
# walking-pointer step inside each PIPELINE_BODY (`lr14 += lr6`, one row per
# iteration) is `lr14 += cr1` directly.
#
# CR register parameters (CR0 = read-only 0, CR1 = read-only 1):
#   cr0  = 0 constant AND input/cyclic-load base (INPUT_BASE_ROW == 0)
#   cr1  = 1 (row stride: pass-counter decrement AND the XMEM row-space unit)
...
# ===========================================================================
    ...
    add                 lr5 lr7 cr1;;    # lr5 = 1 (output row stride)
...
# ---------------------------------------------------------------------------
    ...
    # lr14 = lr0 + 3 rows (channel 3's row); chained cr1 adds since ADD's
    # src_a reads snapshot (can't fold into one word -- see header note).
    add                 lr14 lr0 cr1;
    add                 lr15 lr4 cr9;;
    add                 lr14 lr14 cr1;;
    add                 lr14 lr14 cr1;
...
LAST_EPILOGUE_A:
    ...
    # lr14 = lr0 + 2 rows; needs its own word (ADD src_a reads snapshot).
    add                 lr14 lr14 cr1;;
    INC                 lr4 1;
    ldr_cyclic_mult_reg lr14 cr0 lr8;
    MULT.RC.VE          lr9 lr4 0 lr7 cr15 ;
    ...
    # lr14 = lr0 + 3 rows; own word, same reason (this word's 3 LR sub-slots
    # below are already full with the next-OC preload + output-ptr advance).
    add                 lr14 lr14 cr1;;

    # ACTIVATE.QUANTIZE (identity) activates + quantizes the just-finalized
    # r_acc -> 128 INT8 bytes in post_aaq_reg.  It reads r_acc LIVE,
    # so it does not NEED a cycle after the final acc, but
    # this word's 3 LR sub-slots are already full with the next-OC preload,
    # so ACTIVATE rides here rather than fusing
...
LAST_EPILOGUE_B:
    ...
    # lr14 = lr0 + 3 rows; own word, same reason (the word below is already
    # full with the next-OC preload + output-ptr advance).
    add                 lr14 lr14 cr1;;

    # ACTIVATE.QUANTIZE (identity): activate + quantize finalized r_acc ->
    # 128 INT8 bytes in post_aaq_reg.  Reads r_acc LIVE so
    # it does not NEED a cycle after the final acc, but this word's 3 LR
    ...
    # LR slot) — rides here.  See Half A's identical note.
    INC                 lr4 1;
    add                 lr1 lr1 lr5;
    ldr_cyclic_mult_reg lr14 cr0 lr9;
    ACTIVATE.QUANTIZE identity cr15;;
