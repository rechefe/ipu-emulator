...
class PointwiseConvUnifiedBnActivationApp(IpuApp):
    ...
    Args:
        ...
        bias:         Per-output-channel float32 bias, shape ``[out_channels]``.
                      ...
                      zeros.
        bias_path:    Alternative: raw float32 bias file (the registry binding).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # bias= (array) or bias_path= (raw float32 file, the registry binding).
        self._bias_array = read_bias(bias, getattr(self, "bias_path", None), out_channels)
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # CR0 = read-only 0, CR1 = read-only 1 (cannot be overwritten).
        # INPUT_BASE_ROW is 0, so CR0 serves as both the zero constant and the
        # input/cyclic-load base.  The kernel base (nonzero) lives in CR14
        # (the constant-1 pass decrement uses CR1 directly).
        ...
        # Parameter CR registers (see the .asm header). CR scalars stay
        # integer-only even in wide mode -- none of these carry fractional
        # values, so FP32 mode does not affect them.
...
# Same domain as pointwise_conv_unified except this twin REQUIRES
# apply_relu=True (it unconditionally applies ReLU); bias is optional (zeros
# when absent). The bias tensor itself is a file binding (``bias_path``), never
# part of the query.


def _supports(q):
    if q.kernel_size != 1:
        return f"handles only kernel_size=1 (pointwise); got {q.kernel_size}"
    if q.dilation != 1:
        return f"handles only dilation=1; got {q.dilation}"
    if q.padding != 0:
        return f"handles only padding=0 (no neighbourhood to pad for); got {q.padding}"
    if q.stride != 1:
        return f"handles only stride=1; got {q.stride}"
    if q.groups != 1:
        return f"handles only groups=1 (a 1x1 depthwise conv has no matching app); got {q.groups}"
    if q.in_channels % 8 != 0:
        return f"in_channels ({q.in_channels}) must be a multiple of 8"
    if q.out_channels % 4 != 0:
        return f"out_channels ({q.out_channels}) must be a multiple of 4"
    if q.width > 128:
        return f"width ({q.width}) exceeds 128, the largest width this app supports"
    if not q.apply_relu:
        return (
...
SPEC = universal_spec(
    ...
    explain=lambda q: (
        ...
        f"{q.width} pads internally to "
        f"{'x'.join(map(str, pointwise_pad_shape(q.height, q.width)))}."
    ),
    caveats=lambda q: padding_caveat(q, *pointwise_pad_shape(q.height, q.width)),
)
