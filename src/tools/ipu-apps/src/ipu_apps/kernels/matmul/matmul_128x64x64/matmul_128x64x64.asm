...
# Related: generic matmul harness — not tied to any transformer layer.
#          Same row_loop/MULT.RC.VE template as matmul_128x64x128 (differs
#          only in output stride 256=N*4 vs 512), matmul_64x64x64 (M=64
#          instead of 128), matmul_128x128 (K=128, N=128).
# Tests:   //src/tools/ipu-apps:test_matmul_128x64x64
#
...
#
# Computes a 128x64x64 matrix product: 128 input rows contracted over 64
...
# stride rather than a full 512-byte row). Same accumulation strategy as
# matmul_128x128: the first k-iteration is peeled with ACC.ADD.FIRST to seed
# r_acc, then the remaining K-1 steps accumulate with ACC.ADD.
#
