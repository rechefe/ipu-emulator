"""Transformer matmul 240×240×128 harness (Layer 5 OutProj, 16 tokens).

Computes C[j, t] = sum_k W[j, k] * D[k, t]  for all j in [0, 240), t in [0, 16).

  D: channel-major [240, 16] input  — K channels × 16 tokens
  W: output-major  [240, 240] weights — N_OUT rows × K cols, stored verbatim (no transpose)
...
class MatMul240x240x128App(IpuApp):
    """240x240_x128 transformer matmul harness (Layer 5 OutProj)."""
...
SPEC = matmul_spec(MatMul240x240x128App, m=M, k=K, n=N, role="240x240 transformer matmul")
