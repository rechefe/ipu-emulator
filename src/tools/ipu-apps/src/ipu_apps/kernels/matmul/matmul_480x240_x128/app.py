"""Transformer matmul 480×240×128 harness (Layer 5 FFN1, 16 tokens).

Computes C[j, t] = sum_k W[j, k] * D[k, t]  for all j in [0, 480), t in [0, 16).

  D: channel-major [240, 16] input  — K channels × 16 tokens
  W: output-major  [480, 240] weights — N_OUT rows × K cols, stored verbatim (no transpose)
  C: channel-major [480, 16] output  — N_OUT channels × 16 tokens (FP32 accumulators)
...
from ipu_apps.kernels.matmul.app import matmul_spec
...
# itself (cropping here would break residual_add_16x240's full-row input
# contract).
...
class MatMul480x240x128App(IpuApp):
    """480x240_x128 transformer matmul harness (Layer 5 FFN1)."""
    ...
    def teardown(self, state: "IpuState") -> None:
        ...
        form crop it themselves (see matmul/cases.py for the pattern)."""
...
# The registry's (M, K, N): M is the total token count, N the output channels.
M = N_TOK
N = N_OUT

SPEC = matmul_spec(MatMul480x240x128App, m=M, k=K, n=N, role="480x240 transformer matmul", activation="silu")
