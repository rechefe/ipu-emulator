...
gets a whole 128-element row even though only 16 elements of it are meaningful.
...
This kernel has no checked-in test data; its inputs are generated by its
``cases.py``.
...
VALID_ROW_BYTES = N_TOK * ELEM_BYTES         # N_TOK (16) meaningful elements per row
...
class LayerNorm16x240App(IpuApp):
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        if len(raw) != N_CH * N_TG * ROW_BYTES:
            raise ValueError(
                f"input is {len(raw)} bytes, expected {N_CH * N_TG * ROW_BYTES} "
                f"(one full {LANES}-element FP32 row per channel)"
            )
        state.xmem.write_address(DATA_BASE, bytearray(raw))

        # gamma and beta: 240 values each -> two 128-element rows (128 + 112).
...
SPEC = layernorm_spec(LayerNorm16x240App, channels=N_CH, tokens=N_TOK)
