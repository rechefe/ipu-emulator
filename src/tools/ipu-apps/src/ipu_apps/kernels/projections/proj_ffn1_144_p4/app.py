...
Computes C[p, tg, j, t] = silu(sum_k W[j, k] * D[p, k, tg, t])
  ...
  C[p]: grouped channel-major [2, 288, 128] output per stream -- row (j, tg)
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_ffn1_144_p4.app import ProjFfn1144P4App

    app = ProjFfn1144P4App(
        inst_path="proj_ffn1_144_p4.bin",
        input_path="input.bin",      # (4, 2, 144, 128) FP32: all 4 streams' D
        weights_path="weights.bin",  # (288, 144) FP32
        output_path="output.bin",    # (4, 2, 288, 128) FP32 XMEM rows
    )
...
class ProjFfn1144P4App(ProjectionP4App):
    """144->288 multi-stream (P=4) transformer projection harness (Layer 3 FFN1)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjFfn1144P4App, activation=ACTIVATION)
