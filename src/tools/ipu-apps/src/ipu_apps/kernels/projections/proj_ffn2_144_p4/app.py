...
Computes C[p, tg, j, t] = sum_k W[j, k] * D[p, k, tg, t]
  ...
  D[p]: [2, 288, 128] input per stream (N_TG x K x 128 tokens), staged
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_ffn2_144_p4.app import ProjFfn2144P4App

    app = ProjFfn2144P4App(
        inst_path="proj_ffn2_144_p4.bin",
        input_path="input.bin",      # (4, 2, 288, 128) FP32: all 4 streams' D
        weights_path="weights.bin",  # (144, 288) FP32
...
class ProjFfn2144P4App(ProjectionP4App):
    """288->144 multi-stream (P=4) transformer projection harness (Layer 3 FFN2)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjFfn2144P4App, activation=ACTIVATION)
