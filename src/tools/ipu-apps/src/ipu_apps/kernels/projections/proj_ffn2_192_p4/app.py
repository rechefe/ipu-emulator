...
Computes C[p, j, t] = sum_k W[j, k] * D[p, k, t]
  for all p in [0, 4), j in [0, N_OUT=192), t in [0, N_TOK=64).

  D[p]: channel-major [384, 64] input per stream -- K channels x 64 tokens
  W:    output-major  [192, 384] weights, SHARED across all 4 streams --
        N_OUT rows x K cols, stored verbatim (no transpose)
  C[p]: channel-major [192, 64] output per stream -- N_OUT channels x 64
...
round-trips through the single-stream matmul_192x384_x128 kernel.
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_ffn2_192_p4.app import ProjFFN2192P4App

    app = ProjFFN2192P4App(
        inst_path="proj_ffn2_192_p4.bin",
        input_path="input.bin",      # (4, 1, 384, 64) FP32: all 4 streams' D
        weights_path="weights.bin",  # (192, 384) FP32
        output_path="output.bin",    # (4, 1, 192, 128) FP32 XMEM rows
    )
...
class ProjFFN2192P4App(ProjectionP4App):
    """384->192 multi-stream (P=4) transformer projection harness (Layer 4 FFN2)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjFFN2192P4App, activation=ACTIVATION)
