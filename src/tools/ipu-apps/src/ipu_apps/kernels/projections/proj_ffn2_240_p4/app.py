"""Multi-stream transformer projection matmul harness (Layer 5 FFN2, P=4).

Computes C[p, j, t] = sum_k W[j, k] * D[p, k, t]
  for all p in [0, 4), j in [0, N_OUT=240), t in [0, N_TOK=16).

  D[p]: channel-major [480, 16] input per stream -- K channels x 16 tokens
  W:    output-major  [240, 480] weights, SHARED across all 4 streams --
...
round-trips through the single-stream matmul_240x480_x128 kernel.
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_ffn2_240_p4.app import ProjFFN2240P4App

    app = ProjFFN2240P4App(
        inst_path="proj_ffn2_240_p4.bin",
        input_path="input.bin",      # (4, 1, 480, 16) FP32: all 4 streams' D
        weights_path="weights.bin",  # (240, 480) FP32
...
class ProjFFN2240P4App(ProjectionP4App):
    """480->240 multi-stream (P=4) transformer projection harness (Layer 5 FFN2)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjFFN2240P4App, activation=ACTIVATION)
