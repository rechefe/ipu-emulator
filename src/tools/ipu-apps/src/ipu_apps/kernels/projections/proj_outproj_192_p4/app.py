...
Computes C[p, j, t] = sum_k W[j, k] * D[p, k, t]
  ...
  W:    output-major  [192, 192] weights, SHARED across all 4 streams --
...
round-trips through the single-stream matmul_192x192_x128 kernel.
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_outproj_192_p4.app import ProjOutProj192P4App

    app = ProjOutProj192P4App(
        inst_path="proj_outproj_192_p4.bin",
        input_path="input.bin",      # (4, 1, 192, 64) FP32: all 4 streams' D
        weights_path="weights.bin",  # (192, 192) FP32
...
class ProjOutProj192P4App(ProjectionP4App):
    """192->192 multi-stream (P=4) transformer projection harness (Layer 4 OutProj)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjOutProj192P4App, activation=ACTIVATION)
