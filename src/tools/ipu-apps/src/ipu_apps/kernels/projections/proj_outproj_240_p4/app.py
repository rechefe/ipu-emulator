"""Multi-stream transformer projection matmul harness (Layer 5 OutProj, P=4).

Computes C[p, j, t] = sum_k W[j, k] * D[p, k, t]
  for all p in [0, 4), j in [0, N_OUT=240), t in [0, N_TOK=16).

  D[p]: channel-major [240, 16] input per stream -- K channels x 16 tokens
  W:    output-major  [240, 240] weights, SHARED across all 4 streams --
        N_OUT rows x K cols, stored verbatim (no transpose)
  C[p]: channel-major [240, 16] output per stream -- N_OUT channels x 16
...
round-trips through the single-stream matmul_240x240_x128 kernel.
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_outproj_240_p4.app import ProjOutProj240P4App

    app = ProjOutProj240P4App(
        inst_path="proj_outproj_240_p4.bin",
        input_path="input.bin",      # (4, 1, 240, 16) FP32: all 4 streams' D
        weights_path="weights.bin",  # (240, 240) FP32
        output_path="output.bin",    # (4, 1, 240, 128) FP32 XMEM rows
    )
...
class ProjOutProj240P4App(ProjectionP4App):
    """240->240 multi-stream (P=4) transformer projection harness (Layer 5 OutProj)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjOutProj240P4App, activation=ACTIVATION)
