...
Computes C[p, tg, j, t] = sum_k W[j, k] * D[p, k, tg, t]
  ...
  W:    output-major  [432, 144] weights, SHARED across all 4 streams --
        N_OUT rows x K cols, stored verbatim (no transpose)
  C[p]: grouped channel-major [2, 432, 128] output per stream -- row (j, tg)
...
round-trips through the single-stream matmul_432x144_x128 kernel.
...
Usage::
    ...
    from ipu_apps.kernels.projections.proj_qkv_144_p4.app import ProjQkv144P4App

    app = ProjQkv144P4App(
        inst_path="proj_qkv_144_p4.bin",
        input_path="input.bin",      # (4, 2, 144, 128) FP32: all 4 streams' D
        weights_path="weights.bin",  # (432, 144) FP32
        output_path="output.bin",    # (4, 2, 432, 128) FP32 XMEM rows
    )
...
class ProjQkv144P4App(ProjectionP4App):
    """144->432 multi-stream (P=4) transformer projection harness (Layer 3 QKV)."""

    layout = ProjectionLayout(k=K, n_out=N_OUT, n_tok=N_TOK, n_tg=N_TG)


SPEC = projection_spec(ProjQkv144P4App, activation=ACTIVATION)
