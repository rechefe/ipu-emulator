"""Concat 8x8x160x160: channel-axis concat of two 160-channel tensors (L5).
...
the end of its L5 transformer block (both branches have 160
channels there, so the concrete shape here is 160 + 160 -> 320).
...
Usage::
    ...
    from ipu_apps.kernels.reshape.concat_8x8x160x160.app import Concat8x8x160x160App
"""
...
H   = 8    # spatial height
W   = 8    # spatial width
C_A = 160    # channels in input A (residual branch)
C_B = 160    # channels in input B (fold-branch features)
C_OUT = C_A + C_B
...
class Concat8x8x160x160App(IpuApp):
    """Concat two 160-channel, 8x8-spatial tensors along the channel axis.

    Args:
        inst_path:    Path to assembled instruction binary.
        input_a_path: Path to input A (160 rows of 128 FP32 elements).
        input_b_path: Path to input B (160 rows of 128 FP32 elements).
        output_path:  Optional path to write the 320-row concatenated output.
    """
...
SPEC = concat_spec(Concat8x8x160x160App, h=H, w=W, c_a=C_A, c_b=C_B)
