...
#          pass-through multiply, which is exact) by the
#          round-trip tests in reshape/test.py.
# Tests:   test_fold_32x32x144 (src/tools/ipu-apps/BUILD.bazel)
#
...
#
# STRIPE / TOKEN-GROUP MAPPING -- derived and verified, not assumed:
#   unfold_32x32x144's own cases establish the ground truth: stream s
#   (phase r_ph=s//2, c_ph=s%2) contains
...
#   actual output (not just trusting its reference).
#
...
#   (correctness-first; a cycle-tuned version could cache
