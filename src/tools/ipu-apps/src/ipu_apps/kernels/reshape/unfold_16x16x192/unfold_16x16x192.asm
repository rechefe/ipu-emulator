...
# Related: L4 variant of unfold_32x32x144 (L3) with re-derived geometry (NOT a
#          direct port -- 2 stripes not 8, elements_in_row=16 not 32);
#          unfold_8x8x240 is the L5 sibling; feeds layernorm_64x192
# Tests:   test_unfold_16x16x192 (src/tools/ipu-apps/BUILD.bazel)
#
...
#   stale r_acc values STR_ACC_REG would write.
#   This is the deliberate per-stream layout; a packed [k][p·n] variant
#   that would use the full row is not implemented.
#
...
# the same bundle as the store, or the row lands one slot late.
