...
# Tests:   test_unfold_32x32x144 (src/tools/ipu-apps/BUILD.bazel)
#
...
{# ---------------------------------------------------------------------------
   MULT SNAPSHOT CONTRACT (issue #157)
   ...
   The 32 loads per channel form a fixed sequence -- 8 (stream, token-group)
   groups x 4 stripes -- so the fix is a pure one-bundle rotation: the first
   load is primed before ch_loop, and bundle i multiplies the chunk loaded by
   bundle i-1 while prefetching the chunk bundle i+1 will multiply. The final
   load of a channel wraps to stripe 0 of the NEXT channel, which is why
   src_off advances just before it (LR sub-slots run ahead of LOAD within a
   word, so that ADD cannot share a word with the load that consumes it).

   The body below is generated by a Jinja loop over that sequence so the
   rotation stays consistent across all 32 sites.
--------------------------------------------------------------------------- #}
{% set groups = [
     ("TL", "on",     "on",     "tg0", DST_TL, STRIPE0_TG0, STRIPE1_TG0, STRIPE2_TG0, STRIPE3_TG0),
     ("TL", "on",     "on",     "tg1", DST_TL, STRIPE0_TG1, STRIPE1_TG1, STRIPE2_TG1, STRIPE3_TG1),
     ("TR", "on_inv", "on",     "tg0", DST_TR, STRIPE0_TG0, STRIPE1_TG0, STRIPE2_TG0, STRIPE3_TG0),
     ("TR", "on_inv", "on",     "tg1", DST_TR, STRIPE0_TG1, STRIPE1_TG1, STRIPE2_TG1, STRIPE3_TG1),
     ("BL", "on",     "on_inv", "tg0", DST_BL, STRIPE0_TG0, STRIPE1_TG0, STRIPE2_TG0, STRIPE3_TG0),
     ("BL", "on",     "on_inv", "tg1", DST_BL, STRIPE0_TG1, STRIPE1_TG1, STRIPE2_TG1, STRIPE3_TG1),
     ("BR", "on_inv", "on_inv", "tg0", DST_BR, STRIPE0_TG0, STRIPE1_TG0, STRIPE2_TG0, STRIPE3_TG0),
     ("BR", "on_inv", "on_inv", "tg1", DST_BR, STRIPE0_TG1, STRIPE1_TG1, STRIPE2_TG1, STRIPE3_TG1)
   ] %}
{% set slots = [acc_slot0, acc_slot1, acc_slot2, acc_slot3] %}

    LDR_CYCLIC_MULT_REG {{ acc_slot0 }} {{ ONES_BASE }} {{ acc_slot0 }};;  # r_cyclic[0..127] = 1.0 (dtype-specific)

    LDR_MULT_REG        r0 {{ src_off }} {{ STRIPE0_TG0 }};;               # prime: stripe 0, tg=0 of ch 0
...
ch_loop:
    ...
    # Bundle i multiplies the chunk loaded by bundle i-1 and prefetches the
    # chunk bundle i+1 will multiply (snapshot contract, see header).
{% for g in groups %}
    # -- Stream {{ g[0] }}  ({{ g[3] }}: h={{ g[1] }}, v={{ g[2] }}) --
{%- set gi = loop.index0 %}
{%- for s in range(4) %}
{#- The chunk to PREFETCH is the next entry in the flat 32-load sequence. #}
{%- set ni = (gi * 4 + s + 1) % 32 %}
{%- set ng = groups[ni // 4] %}
{%- set nstripe = ng[5 + (ni % 4)] %}
{%- if ni == 0 %}
    # Last load of the channel wraps to stripe 0 of the NEXT channel, so
