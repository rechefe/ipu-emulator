...
def _supports(**params):
    ...
    if bad:
        return no(bad)
    if (q.channels, q.tokens) != (N_CH, N_TPG):
        return no(
            f"handles exactly (channels={N_CH}, tokens={N_TPG}); "
...
def _explain(**params):
    return f"exact match: (channels={N_CH}, tokens={N_TPG})."
...
SPEC = KernelSpec(
    ...
    op="layernorm",
    variant="128x16",
    app_class=LayerNorm128x16App,
    asm="layernorm_128x16.asm",
    requires=("shape",),
    tags=("fp32-wide",),
    supports=_supports,
    build=_build,
