...
def _supports(**params):
    ...
    if bad:
        return no(bad)
    if (q.channels, q.tokens) != (N_CH, N_TOK):
        return no(
            f"handles exactly (channels={N_CH}, tokens={N_TOK}); "
...
def _explain(**params):
    return f"exact match: (channels={N_CH}, tokens={N_TOK})."
...
SPEC = KernelSpec(
    ...
    op="layernorm",
    variant="16x240",
    app_class=LayerNorm16x240App,
    asm="layernorm_16x240.asm",
    requires=("shape",),
    tags=("fp32-wide",),
    supports=_supports,
    build=_build,
