"""FP32 debug-input generation for layernorm_64x192.
...
``test/test_layernorm_64x192_wide.py`` so that
``python -m ipu_apps.layernorm.layernorm_64x192`` exercises the kernel the way its test
does.
...
from ipu_apps.layernorm.layernorm_64x192 import N_CH, N_TOK, LANES
...
def generate(out_dir: Path) -> dict[str, Path]:
    ...
    x is one row per channel (N_TG=1), N_TOK valid lanes, tail lanes zero.
    """
