# LayerNorm 64×192 (Layer 4): per-PE normalization across 192 channels, 64 tokens (1 tg × 64)
#
# Wide-vector debug mode (512 B per row = 128 × FP32).
# Same algorithm and VLIW patterns as layernorm_256x144. Key differences:
#   - N_CH=192, N_TG=1 → NO outer tg loop; the whole kernel runs once
#   - One channel per XMEM row: row for ch is DATA_BASE + ch (rows, issue #179),
#     so the per-channel data stride IS the row stride (1). The ancestor's
#     separate 2-row data_stride is gone along with N_TG=2.
#   - Only lanes 0..63 carry tokens; lanes 64..127 are zero padding that rides
#     through the arithmetic. The reduction is across CHANNELS (the 192 rows),
#     independently per lane, so padding lanes simply normalize zeros and are
#     cropped by the consumer. Rows are never shared between channels.
#   - γ/β have 192 values (>128) → reload r0/r1 at ch=128 in step 6, and
#     sub-loop B covers 64 channels (192-128), not the ancestor's 16.
#   - NEG_INV_N = FP32(-1/192), INV_N = FP32(1/192)  (over channels, not tokens)
#
...
{% set ch_limit       = "lr6"  %}  {# 192 = N_CH loop bound #}
...
{% set sub_bound      = "lr15" %}  {# step-6 sub-loop bound (128, then 64) #}
...
{% set NEG_INV_N_BASE = "cr4"  %}  {# FP32(-1/192) #}
{% set INV_N_BASE     = "cr5"  %}  {# FP32(1/192) #}
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    LDR_CYCLIC_MULT_REG {{ read_ptr }} {{ DATA_BASE }} {{ rc_slot0 }}; ADD {{ read_ptr }} {{ read_ptr }} {{ row_stride }};;   # prime x[ch=0]

    # Peeled first ch (ch=0): ACC.ADD.FIRST seeds r_acc.
    MULT.RC.VV {{ rc_slot0 }} r0 0 {{ mask_shift }} {{ DSTRUCT }}; ACC.ADD.FIRST;;
    LDR_CYCLIC_MULT_REG {{ read_ptr }} {{ DATA_BASE }} {{ rc_slot0 }}; ADD {{ read_ptr }} {{ read_ptr }} {{ row_stride }}; ADD {{ ch_index }} {{ ch_index }} {{ ONE }}; BLT {{ ch_index }} {{ ch_limit }} step1_loop;;
    B       step1_done;;
step1_loop:
...
#
# r0 = ONES, r1 = -μ. read_ptr = data read ptr. write_ptr = centered write ptr (-1).
# Three cycles per ch.
# ─────────────────────────────────────────────────────────────────────────────
...
step2_loop:
    ...
    ACTIVATE.QUANTIZE identity {{ DSTRUCT }}; STR_POST_AAQ_REG {{ write_ptr }} {{ CENTERED_BASE }}; ADD {{ write_ptr }} {{ write_ptr }} {{ row_stride }}; LDR_CYCLIC_MULT_REG {{ read_ptr }} {{ DATA_BASE }} {{ rc_slot0 }}; ADD {{ read_ptr }} {{ read_ptr }} {{ row_stride }};;
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 4: variance = (1/N_CH) × Σ(x-μ)²;  1/σ = ACTIVATE rsqrt
# ─────────────────────────────────────────────────────────────────────────────
...
#
# r0 = 1/σ. read_ptr = -1 (load ptr), write_ptr = 0 (store ptr, trails by a row).
...
#
# N_CH=192 > 128: γ/β span two rows.
#   Row 0: γ[0..127],   β[0..127]
#   Row 1: γ[128..191], β[128..191] (rest zero-padded by harness)
#
# Sub-loop A: ch=0..127   → r0=γ row0, r1=β row0, gamma_idx=fixed_idx 0..127
# Sub-loop B: ch=128..191 → reload r0=γ row1, r1=β row1, gamma_idx=0..63
#
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ch_limit currently = 192; use a separate bound sub_bound=128 for sub-loop A
...
step6A_loop:
    ...
    ACTIVATE.QUANTIZE identity {{ DSTRUCT }}; STR_POST_AAQ_REG {{ write_ptr }} {{ OUTPUT_BASE }}; ADD {{ write_ptr }} {{ write_ptr }} {{ row_stride }}; ADD {{ gamma_idx }} {{ gamma_idx }} {{ ONE }}; ADD {{ beta_idx }} {{ beta_idx }} {{ ONE }};;
    ...
    # ---- Sub-loop B: ch=128..191 (64 channels) ----
    LDR_MULT_REG        r0 {{ row_stride }} {{ GAMMA_BASE }};;    # r0 ← γ row 1 (offset = 1 row)
    ...
    # bound for sub-loop B: 64 channels (192-128), built by doubling CR1: 1→2→4→8→16→32→64
    SET     {{ sub_bound }} {{ ONE }};;
    ...
    ADD     {{ sub_bound }} {{ sub_bound }} {{ sub_bound }};;     # 32
    ADD     {{ sub_bound }} {{ sub_bound }} {{ sub_bound }};;     # 64
