"""Generate test data for the matmul_128x64x64 app.
...
Usage::
    ...
    bazel run //src/tools/ipu-apps:gen_matmul_128x64x64_test_data
"""
...
M = 128   # rows of A
K = 64    # cols of A / cols of W per row
N = 64    # rows of W (output neurons)
...
def _reference_matmul(
    ...
    Input layout (A):   row-major, A[m][k] at byte m*K + k  (K=64 bytes per row).
    Weights layout (W): output-major, W[n][k] at byte n*K + k  (N rows × K cols).
...
def main() -> None:
    ...
    print("Generating matmul_128x64x64 test data...")
