"""Generate test data for the matmul_192x192_x128 app.

Usage::

    uv run python src/ipu_apps/matmul_192x192_x128/gen_test_data.py
"""
...
def _reference_matmul(data_bytes: bytes, weights_bytes: bytes, dtype: DType) -> bytes:
    ...
    D[k][tok] at byte k*N_TOK + tok; W[j][k] at byte j*K + k.
    C[j][tok] at word j*N_TOK + tok (channel-major, packed).
    """
...
def main() -> None:
    ...
    print("Generating matmul_192x192_x128 test data...")
