...
# Scope:   single-stream
# Layout:  unpacked
# Shape:   M=256 tokens (2 token groups x 128), K=144, N=288
# Status:  validated
# Related: L3 FFN1 (single-stream, 2x output expansion). Body identical to
#          matmul_144x144_x128 (§4.2 in kernel_docs/L3_kernel_reference.md
#          calls this "2x the outputs" — only N_OUT and OUTPUT_BASE differ).
#          Sibling family: matmul_432x144_x128 (fused QKV), matmul_144x288_x128
#          (FFN2). Consumed by proj_ffn1_144_p4 (all 4 streams, one
#          invocation, same silu activation).
# Tests:   //src/tools/ipu-apps:test_matmul_288x144_x128_wide
#
# Computes the L3 FFN1 matmul: 144 input channels expanded to 288 hidden
# channels, over 256 tokens (two 128-token groups). Same two-k-loop
# contraction as matmul_144x144_x128 (k=0..127 from r0, k=128..143 from r1),
# with the first iteration peeled via ACC.ADD.FIRST to seed r_acc and the
# rest accumulated with ACC.ADD, once per token group. Unlike its siblings,
# the store applies ACTIVATE.QUANTIZE silu rather than identity — FFN1 is
# followed by FFN2 with no other nonlinearity in between, so the activation
# lives here.
#
...
k_loop2_tg0:
    ...
    ACTIVATE.QUANTIZE silu {{ DSTRUCT }};
