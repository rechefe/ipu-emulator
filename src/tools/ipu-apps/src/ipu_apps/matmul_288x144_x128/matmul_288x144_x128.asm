...
# C: channel-major [288 out_ch, 2 tg, 128 tokens], FP32 accumulators
...
#   cr3 = OUTPUT_BASE           (0x30000 — must be past weights end 0x22000)
#
# LRs:
...
#   Slot order: XMEM → MULT → ACC → LR → COND within each compound instruction.
#   XMEM reads lr4 and MULT reads lr5 BEFORE LR increments them in the same cycle.
#   Initialise both one step behind the true first value so the pipeline aligns:
#     lr4 = -256: dummy XMEM reads DATA_BASE−256 (before region → zeros)
#     lr5 = -1:   dummy MULT reads r_cyclic[511] = 0 → contributes 0 to accumulator
#   tg=0: set lr4=-256 → first real load = D[k=0, tg=0] at DATA_BASE+0
#   tg=1: set lr4=-128 → first real load = D[k=0, tg=1] at DATA_BASE+128
#          (−256 + 128: tg=1 data starts 128 B into each 256 B channel block)
#
# Memory layout:
#   DATA:    144 channels × 256 B = 36 864 B  (0x00000..0x08FFF)
#   WEIGHTS: 288 rows × 256 B    =  73 728 B  (0x10000..0x21FFF)
#   OUTPUT:  288×2 rows × 512 B  = 294 912 B  (0x30000..0x77FFF)
...
j_loop:
    ...
    # -- token group 0 -------------------------------------------------------
    reset_acc;;
    set                 lr4 -256;;       # startup offset: first real load will be at DATA_BASE+0
    set                 lr5 -1;;         # startup offset: first real cyclic read will be index 0
