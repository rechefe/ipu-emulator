"""Generate test data for the matmul_576x192_x128 app (Layer 4 QKV, 64 tokens).
...
Usage::
    ...
    uv run python src/ipu_apps/matmul_576x192_x128/gen_test_data.py
"""
...
def _reference_matmul(data_bytes: bytes, weights_bytes: bytes, dtype: DType) -> bytes:
    ...
    Data layout:    D[k][tok] at byte k*N_TOK + tok  (channel-major).
    Weights layout: W[j][k]   at byte j*K + k         (output-major).
    Output layout:  C[j][tok] at word j*N_TOK + tok   (channel-major, packed).
...
def main() -> None:
    ...
    print("Generating matmul_576x192_x128 test data...")
