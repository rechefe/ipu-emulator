"""Debug runner for matmul_128x64x128.
...
``test/test_matmul_128x64x128_wide.py`` is the reference for correctness.

Usage::

    MATMUL_128X64X128_INST_BIN=/tmp/matmul_128x64x128.bin \
    uv run python -m ipu_apps.matmuls.matmul_128x64x128
"""
...
from ipu_apps.matmuls.matmul_128x64x128 import MatMul128x64x128App
from ipu_apps.matmuls.matmul_128x64x128.gen_debug_data import generate


def main() -> None:
    _INST_BIN = Path(os.environ["MATMUL_128X64X128_INST_BIN"])

    work = Path(tempfile.mkdtemp(prefix="matmul_128x64x128_"))
