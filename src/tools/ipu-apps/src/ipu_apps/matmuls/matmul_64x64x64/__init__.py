...
Computes C = A × W^T where:
  A: (M=64, K=64) input   — K elements per row in file; zero-padded to LANES in XMEM
  W: (N=64, K=64) weights — output-major, W[n][k] (row n = all K inputs for output n)
  C: (M=64, N=64) output  — N accumulators per matrix row
...
class MatMul64x64x64App(IpuApp):
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # architecture — writing anything else now raises EmulatorError (issue #230 / PR #231). INPUT_BASE_ROW is 0, so
...
def _explain(**params):
    return f"(M, K, N) == {(M, K, N)} exactly: the fixed-shape 64x64x64 matmul kernel."
...
SPEC = KernelSpec(
    ...
    op=OP,
    variant="64x64x64",
    app_class=MatMul64x64x64App,
    asm="matmul_64x64x64.asm",
