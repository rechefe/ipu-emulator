"""Debug runner for the pointwise convolution app.

Usage::

    bazel run //src/tools/ipu-apps:pointwise_conv -- --dtype INT8
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run pointwise conv with debug CLI")
    ...
    inst_path = Path(os.environ["PCONV_INST_BIN"])
    data_dir = Path(os.environ["PCONV_DATA_DIR"])
