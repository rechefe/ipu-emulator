"""Generate test data for the 128x128x16→64 pointwise convolution app.
...
Kernel layout: kernel[oc * IN_CHANNELS + ic] for oc=0..63, ic=0..15
...
Usage::
    ...
    bazel run //src/tools/ipu-apps:gen_pointwise_conv_128x128_16to64_test_data
"""
...
def _reference_pointwise_conv(
    ...
    Input layout: row-group interleaved, 1 row per 128-byte chunk.
      Row-group rg, channel ic, element idx (col c):
        offset = rg * 2048 + ic * 128 + idx

    Output layout: row-group interleaved, 128 elements per store (1 row x 128 cols).
      Row-group rg, output channel oc, element idx:
        byte_offset = (rg * 64 + oc) * 128 * 4 + idx * 4

    Returns 128*64*128 accumulator words packed as bytes = 4194304 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROW_GROUPS * IN_CHANNELS * 128  # 262144
