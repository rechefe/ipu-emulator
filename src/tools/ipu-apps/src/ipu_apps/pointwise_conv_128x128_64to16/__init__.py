"""Pointwise (1x1) convolution harness — 128x128x64→16 variant.

128x128x64 input, 1x1x64x16 kernel, 128x128x16 output.
Since spatial width (128) = SIMD width (128), 1 spatial row is packed
per 128-byte chunk.  64 input channels, 16 output channels with
1024-byte kernel split across 4 kernel groups of r0+r1 (256 bytes each).

Usage::

    from ipu_apps.pointwise_conv_128x128_64to16 import PointwiseConv128x128_64to16App

    app = PointwiseConv128x128_64to16App(
        inst_path="pointwise_conv_128x128_64to16.bin",
...
class PointwiseConv128x128_64to16App(IpuApp):
    """Pointwise (1x1) convolution application harness — 128x128x64→16 variant.

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input image binary (128x128x64 bytes, interleaved by row-group).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input image (128 row-groups x 64 channels x 128 bytes = 1048576 bytes)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 128 row-groups x 16 channels, each 512 bytes
