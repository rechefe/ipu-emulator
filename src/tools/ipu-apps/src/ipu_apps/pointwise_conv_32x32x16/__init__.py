"""Pointwise (1x1) convolution harness — 32x32x16 variant.

32x32x16 input, 1x1x16x16 kernel, 32x32x16 output.
Since spatial width (32) < SIMD width (128), 4 spatial rows are packed
per 128-byte chunk.  16 input/output channels with 256-byte kernel split
across r0 (oc 0-7) and r1 (oc 8-15).

Usage::

    from ipu_apps.pointwise_conv_32x32x16 import PointwiseConv32x32x16App

    app = PointwiseConv32x32x16App(
        inst_path="pointwise_conv_32x32x16.bin",
...
KERNEL_BYTES = IN_CHANNELS * OUT_CHANNELS  # 256
...
class PointwiseConv32x32x16App(IpuApp):
    """Pointwise (1x1) convolution application harness — 32x32x16 variant.

    Args:
        inst_path:   Path to assembled instruction binary.
        input_path:  Path to input image binary (32x32x16 bytes, interleaved by row-group).
        kernel_path: Path to kernel binary (256 bytes).
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # Load input image (8 row-groups x 16 channels x 128 bytes = 16384 bytes)
        input_data = self.input_path.read_bytes()
        state.xmem.write_address(INPUT_BASE_ADDR, input_data)

        # Load kernel (256 bytes, already aligned to 128)
    ...
    def teardown(self, state: "IpuState") -> None:
        if self.output_path is not None:
            # Output: 8 row-groups x 16 channels, each 512 bytes
