"""Debug runner for the 32x32x16 pointwise convolution app.

Usage::

    bazel run //src/tools/ipu-apps:pointwise_conv_32x32x16 -- --dtype INT8
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run 32x32x16 pointwise conv with debug CLI")
    ...
    inst_path = Path(os.environ["PCONV16_INST_BIN"])
    data_dir = Path(os.environ["PCONV16_DATA_DIR"])
