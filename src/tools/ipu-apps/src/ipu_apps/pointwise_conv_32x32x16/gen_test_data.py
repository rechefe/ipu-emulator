"""Generate test data for the 32x32x16 pointwise convolution app.

Creates input, kernel, and golden-output binaries for each dtype,
matching the IPU's arithmetic exactly via ipu_math.

Input layout: interleaved by row-group (4 spatial rows per 128-byte chunk).
  Row-group rg, channel ic, local-row lr, col c:
    offset = rg * ROW_GROUP_STRIDE + ic * 128 + lr * COLS + c

Kernel layout: kernel[oc * IN_CHANNELS + ic] for oc=0..15, ic=0..15

Output layout: interleaved by row-group.
  Row-group rg, output channel oc, local-row lr, col c:
    byte_offset = (rg * OUT_CHANNELS + oc) * 128 * 4 + (lr * COLS + c) * 4

Usage::

    bazel run //src/tools/ipu-apps:gen_pointwise_conv_32x32x16_test_data
"""
...
ROW_GROUP_STRIDE = IN_CHANNELS * 128  # 2048
...
def _reference_pointwise_conv(
    ...
    For each output channel oc, spatial position (r, c):
      acc = sum over ic in [0..15]: kernel[oc*16 + ic] * input[ic][r][c]

    Input layout: row-group interleaved, 4 rows per 128-byte chunk.
      Row-group rg, channel ic, local-row lr, col c:
        offset = rg * 2048 + ic * 128 + lr * 32 + c

    Output layout: row-group interleaved, 128 elements per store (4 rows x 32 cols).
      Row-group rg, output channel oc, element idx (lr*32+c):
        byte_offset = (rg * 16 + oc) * 128 * 4 + idx * 4

    Returns 8*16*128 accumulator words packed as bytes = 65536 bytes.
    """
    ...
    for rg in range(ROW_GROUPS):
        for oc in range(OUT_CHANNELS):
            for elem in range(SIMD_WIDTH):  # 128 elements = 4 rows x 32 cols
                acc: int | float = 0
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROW_GROUPS * IN_CHANNELS * 128  # 16384
    kernel_size = IN_CHANNELS * OUT_CHANNELS  # 256
    ...
    # Write kernel (256 bytes, naturally aligned to 128)
