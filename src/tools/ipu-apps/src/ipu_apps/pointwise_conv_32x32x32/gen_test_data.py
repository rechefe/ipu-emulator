"""Generate test data for the 32x32x32 pointwise convolution app.
...
Kernel layout: kernel[oc * IN_CHANNELS + ic] for oc=0..31, ic=0..31
...
Usage::
    ...
    bazel run //src/tools/ipu-apps:gen_pointwise_conv_32x32x32_test_data
"""
...
ROW_GROUP_STRIDE = IN_CHANNELS * 128  # 4096
...
def _reference_pointwise_conv(
    ...
    For each output channel oc, spatial position (r, c):
      acc = sum over ic in [0..31]: kernel[oc*32 + ic] * input[ic][r][c]

    Input layout: row-group interleaved, 4 rows per 128-byte chunk.
      Row-group rg, channel ic, element idx (lr*32+c):
        offset = rg * 4096 + ic * 128 + idx

    Output layout: row-group interleaved, 128 elements per store (4 rows x 32 cols).
      Row-group rg, output channel oc, element idx:
        byte_offset = (rg * 32 + oc) * 128 * 4 + idx * 4

    Returns 8*32*128 accumulator words packed as bytes = 131072 bytes.
    """
...
def _generate_for_dtype(out_dir: Path, dtype: DType, dtype_name: str) -> None:
    ...
    input_size = ROW_GROUPS * IN_CHANNELS * 128  # 32768
