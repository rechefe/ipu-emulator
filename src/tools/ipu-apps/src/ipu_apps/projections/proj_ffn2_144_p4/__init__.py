"""Multi-stream transformer projection matmul harness (Layer 3 FFN2, P=4).
...
round-trips through the single-stream matmul_288x144_x128 kernel.
...
Usage::
    ...
    from ipu_apps.projections.proj_ffn2_144_p4 import ProjFfn2144P4App

    app = ProjFfn2144P4App(
        inst_path="proj_ffn2_144_p4.bin",
...
# ---------------------------------------------------------------------------
# Wide-vector FP32 only -- see matmul_288x144_x128/__init__.py for the full
...
def _load_stream_data(state: "IpuState", data_path: str | Path, stream: int) -> None:
    ...
    matmul_288x144_x128's own _load_data contiguous layout, just staged per
...
class ProjFfn2144P4App(IpuApp):
    """288x144 multi-stream (P=4) transformer projection harness (Layer 3 FFN2)."""
...
def _explain(**params):
    return (
        f"k == {K} and n_out == {N_OUT} exactly: the Layer 3 FFN2 multi-stream "
...
SPEC = KernelSpec(
    ...
    op="projection",
    variant="ffn2_144",
    app_class=ProjFfn2144P4App,
    asm="proj_ffn2_144_p4.asm",
    requires=("k", "n_out"),
