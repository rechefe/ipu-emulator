...
Usage::
    ...
    from ipu_apps.projections.proj_outproj_144_p4 import ProjOutproj144P4App
...
# ---------------------------------------------------------------------------
# Wide-vector FP32 only -- see matmul_144x144_x128/__init__.py for the full
...
def _load_stream_data(state: "IpuState", data_path: str | Path, stream: int) -> None:
    ...
    matmul_144x144_x128's own _load_data contiguous layout, just staged per
...
class ProjOutproj144P4App(IpuApp):
    """144x144 multi-stream (P=4) transformer projection harness (Layer 3 OutProj)."""
...
def _explain(**params):
    return (
        f"k == {K} and n_out == {N_OUT} exactly: the Layer 3 OutProj multi-stream "
...
SPEC = KernelSpec(
    ...
    op="projection",
    variant="outproj_144",
    app_class=ProjOutproj144P4App,
    asm="proj_outproj_144_p4.asm",
    requires=("k", "n_out"),
