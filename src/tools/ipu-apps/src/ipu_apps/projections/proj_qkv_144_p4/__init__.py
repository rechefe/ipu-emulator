"""Multi-stream transformer projection matmul harness (Layer 3 QKV, P=4).

Computes C[p, tg, j, t] = sum_k W[j, k] * D[p, k, tg, t]
  for all p in [0, 4), j in [0, N_OUT    = 432), tg in [0, N_TG=2), t in [0, 128).
  ...
  W:    output-major  [144, 432] weights, SHARED across all 4 streams --
...
round-trips through the single-stream matmul_144x432_x128 kernel.
...
Usage::
    ...
    from ipu_apps.projections.proj_qkv_144_p4 import ProjQkv144P4App

    app = ProjQkv144P4App(
        inst_path="proj_qkv_144_p4.bin",
...
from ipu_apps.projections._spec_support import (
    FIXED_STREAMS_ONLY,
    exact_shape_reason,
    positive_dims,
    projection_query,
...
# ---------------------------------------------------------------------------
# Wide-vector FP32 only -- see matmul_144x432_x128/__init__.py for the full
...
def _load_stream_data(state: "IpuState", data_path: str | Path, stream: int) -> None:
    ...
    matmul_144x432_x128's own _load_data contiguous layout, just staged per
...
class ProjQkv144P4App(IpuApp):
    """144x432 multi-stream (P=4) transformer projection harness (Layer 3 QKV)."""
...
# is the single source of truth for this kernel's domain -- an exact (k,
# n_out) match, since the .asm is generated for this one shape.


def _supports(**params):
    q = projection_query(k=params["k"], n_out=params["n_out"], n_streams=params.get("n_streams", N_STREAM))
    bad = positive_dims(q)
    if bad:
        return no(bad)
    reason = exact_shape_reason(q, K, N_OUT)
    if reason:
        return no(reason)
    return yes()
...
def _explain(**params):
    return (
        f"k == {K} and n_out == {N_OUT} exactly: the Layer 3 QKV multi-stream "
        f"(P={N_STREAM}) projection kernel."
    )
...
SPEC = KernelSpec(
    ...
    op="projection",
    variant="qkv_144",
    app_class=ProjQkv144P4App,
    asm="proj_qkv_144_p4.asm",
    requires=("k", "n_out"),
    tags=("fp32-wide", "multi-stream"),
    supports=_supports,
    build=_build,
    explain=_explain,
    caveats=lambda **params: (FIXED_STREAMS_ONLY,),
    bundle=lambda **params: projection_query(
        k=params["k"], n_out=params["n_out"], n_streams=params.get("n_streams", N_STREAM)
    ).bundle,
