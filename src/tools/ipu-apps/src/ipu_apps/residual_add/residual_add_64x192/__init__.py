...
from ipu_apps.residual_add._spec_support import (
    WIDE_VECTOR_ONLY,
    positive_dims,
    residual_add_query,
)
...
class ResidualAdd64x192App(IpuApp):
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # writing anything else now raises EmulatorError (issue #230 / PR #231). B_BASE is therefore on CR9 (free).
...
def _supports(**params):
    ...
    if bad:
        return no(bad)
    if (q.tokens, q.channels) != (N_TOK, N_CH):
        return no(
            f"handles exactly (tokens={N_TOK}, channels={N_CH}); "
...
def _explain(**params):
    return f"exact match: (tokens={N_TOK}, channels={N_CH})."
...
SPEC = KernelSpec(
    ...
    op="residual_add",
    variant="64x192",
    app_class=ResidualAdd64x192App,
    asm="residual_add_64x192.asm",
    requires=("shape",),
    tags=("fp32-wide",),
    supports=_supports,
    build=_build,
