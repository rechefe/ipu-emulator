"""Mixer-residual test harness — the first residual of ``Block.forward``.

Implements::

    x = x + self.drop_path(self.gamma_1 * self.mixer(self.norm1(x)))

``skip`` is the block input x, ``branch`` is the mixer output after
``out_proj``; both are token view ``(L, d_model)``. ``DropPath`` is
``nn.Identity`` at inference and ``mamba_vision_T`` sets no ``layer_scale``,
so ``gamma_1`` is 1 and this is an element-wise add with an INT8 clamp on the
way out.
...
class ResidualMixerApp(IpuApp):
    """Mixer-residual application harness.

    Args:
        inst_path:   Path to assembled instruction binary.
        inputs_path: Path to the skip tensor (the block input), token view.
        branch_path: Path to the mixer output being added in, token view.
        output_path: Optional path to write output.
        stage:       ``'stage3'`` or ``'stage4'`` of ``mamba_vision_T``.
        gamma:       ``layer_scale`` coefficient ``gamma_1``. ``mamba_vision_T``
            sets no layer_scale, so ``Block.gamma_1`` is the int 1; pass
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if branch_path is None:
            raise ValueError("residual_mixer needs both addends: pass branch_path=...")
    ...
    def setup(self, state: "IpuState") -> None:
        ...
        # the skip term, CR6 carries gamma_1 for the branch term, so the
