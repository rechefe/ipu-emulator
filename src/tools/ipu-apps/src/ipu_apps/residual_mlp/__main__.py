"""Debug runner for the MLP-residual app.

Usage::

    bazel run //src/tools/ipu-apps:residual_mlp -- --stage stage3
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run MLP-residual with debug CLI")
    ...
    inst_path = Path(os.environ["RESIDUAL_MLP_INST_BIN"])
    data_dir = Path(os.environ["RESIDUAL_MLP_DATA_DIR"]) / args.stage
