{#- ==========================================================================
    ...
    lane. Each lane is an independent column, so the per-column max/sum are full
    128-lane vectors -- NO AGG, NO fan-out. The running reduce down the rows is
    ACC.MAX (Pass 1) / ACC.ADD (Pass 3); the subtract (Pass 2) and normalise
    (Pass 4) are lane-aligned vector ops against the resident cmax/rvec chunks.
...
{#- The cmax[c] subtract is a FULL lane-aligned vector (no element select).-#}
{#- ===================================================================== -#}
    SET {{lr_chunk}} cr0 ;
    SET {{lr_coff}}  cr0 ;;
    SET {{lr_svoff}} cr0 ;;
...
{#-   r_cyclic = num[r,c]; R1 = rvec[c]; lane-aligned vector x vector.      -#}
{#- ===================================================================== -#}
    SET {{lr_chunk}} cr0 ;
    SET {{lr_coff}}  cr0 ;;
    SET {{lr_svoff}} cr0 ;;
