...
``c = log2(e)`` rides in a resident 128-lane vector ``C_VEC``; the 1.0 scalar is
the read-only CR1.

Per-row scalars (``maxvec[r]`` / ``rvec[r]``) hold one value per row in a single
128-lane vector, so this version processes up to 128 rows in one group (no group
...
512 B chunks at ``BASE + r*cpr*512``; the tail chunk's unused lanes
...
class SoftmaxRowsLongApp(IpuApp):
    ...
    def _pack_input(self) -> bytes:
        ...
        chunk's lanes [tail..128) stay zero.
        """
    ...
    def teardown(self, state: "IpuState") -> None:
        ...
        # Un-pack: drop each tail chunk's padding lanes, emit row-major rows x N.
...
def _supports(**params):
    ...
    if q.n <= LANES:
        return no(
            f"splits a row across several {LANES}-lane chunks, so it needs a "
