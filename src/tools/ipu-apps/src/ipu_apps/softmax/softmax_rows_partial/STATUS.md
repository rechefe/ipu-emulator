...
shared `r_acc`. `MULT.RC.VE` always produces 128 real lanes -- only
...
`acc.add.first` write/accumulate all 128 lanes unconditionally, so each
partition's out-of-range lanes got baked into `r_acc` and corrupted the
...
that partition's own lanes, and `p` ranges over `0..P-1` where P is only
...
shifts by up to +/-3 bits -- far too little to span a 16..128-lane
...
2. **`.asm`**: `LDR_MULT_MASK_REG` loads the whole 8-slot image once per
   ...
   the shared `p4_drain`.
3. With each partition's `MULT.RC.VE` now masked to exactly its own lane
   range (every other lane -> `PadMode.ZERO`), `acc.add`/`acc.add.first`
...
The real constraint is that `maxvec`/`rvec` are single 128-lane vectors with
