"""FP32 debug-input generation for fold_32x32x144.

Fold's natural input is unfold_32x32x144's output, so this generator builds a
random spatial tensor, runs the REAL unfold_32x32x144 kernel binary on it (via
...
Self-contained aside from importing unfold_32x32x144 itself (deliberately, to
...
from ipu_apps.unfold.unfold_32x32x144 import H, W, C, N_STRIPES, LANES, Unfold32x32x144App

_SEED = 0x032

_UNFOLD_ASM_PATH = (
    Path(__file__).resolve().parents[1] / "unfold_32x32x144" / "unfold_32x32x144.asm"
)
...
def _spatial_to_nhcw_striped(x: np.ndarray) -> np.ndarray:
    """[C, H, W] -> NHCW-striped rows, matching unfold_32x32x144's own input recipe."""
...
def generate(out_dir: Path) -> dict[str, Path]:
    ...
    # Run the real unfold_32x32x144 kernel to get fold's natural input.
    with tempfile.TemporaryDirectory() as tmp:
        unfold_bin = Path(tmp) / "unfold_32x32x144.bin"
