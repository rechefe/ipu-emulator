...
``python -m ipu_apps.unfold.unfold_8x8x240`` exercises the kernel the way its test does.
...
from ipu_apps.unfold.unfold_8x8x240 import H, W, C, pack_input_rows
...
def pack_stripe(x: np.ndarray) -> np.ndarray:
    ...
    Thin wrapper over :func:`ipu_apps.unfold.unfold_8x8x240.pack_input_rows` -- the
