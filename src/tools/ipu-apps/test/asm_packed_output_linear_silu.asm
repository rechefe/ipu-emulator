...
#          asm_packed_output_linear_silu_p4.asm (L4 port)
# Tests:   exercised via test_full_layer_l5_packed.py
#
# ACTIVATE.QUANTIZE silu instead of identity, matching every production
# FFN1 kernel (proj_ffn1_*_p4.asm) -- GELU does not exist anywhere in this
# codebase (no kernel uses it; it exists only as an activation-fn ID in
# the ISA) despite the original task brief calling for it. Per explicit
# user direction, this uses silu to match production rather than acting
# on the brief's literal wording; see the task report for the full
# GELU-vs-SiLU discrepancy writeup.
#
