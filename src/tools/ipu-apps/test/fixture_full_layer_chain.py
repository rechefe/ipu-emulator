...
def run_qk_scores_query_major_one_head(App, *, inst_bin: Path, q_head: np.ndarray,
    ...
    from ipu_apps.attention.qk_scores_16x60 import S_BASE, OUTPUT_ROW_BYTES, N_TG
...
def run_attn_v_query_major_all_blocks(App, *, inst_bin: Path, p_blocks: np.ndarray,
    ...
    from ipu_apps.attention.attn_v_64x48 import OBASE, OUTPUT_ROW_BYTES as O_ROW_BYTES
...
def run_attn_v_query_major_one_stream(App, *, inst_bin: Path, p_bytes_per_head: list[bytes],
    ...
    from ipu_apps.attention.attn_v_16x60 import OBASE, O_CHAN_BYTES, N_CHAN
...
# ---------------------------------------------------------------------------
# In-process assembly for the layer-chain apps themselves (layernorm,
# projections, attention, residual_add). Replaces the old convention of
# reading a Bazel-genrule-assembled .bin path out of an env var: the test
# assembles its own instruction binaries, exactly like every other
# modernized standalone test in this directory. Assembled once per process
# (module-level cache) since each .asm is fixed and every call in a test run
# reuses the same binary -- see test_softmax_kernels_vs_stub.py's _assemble
# for why reset_labels() is required before each assembly.
# ---------------------------------------------------------------------------

_APP_INST: dict[str, Path] = {}


def app_inst(family: str, name: str) -> Path:
    """Assemble (once) and cache ``ipu_apps/<family>/<name>/<name>.asm``."""
    if name in _APP_INST:
        return _APP_INST[name]
    ...
    asm_path = (
        ...
        / f"src/ipu_apps/{family}/{name}/{name}.asm"
    )
    tmp_dir = Path(tempfile.mkdtemp(prefix=f"full_layer_{name}_"))
    inst_path = tmp_dir / f"{name}.bin"
    reset_labels()
    assemble_to_bin_file(asm_path.read_text(encoding="utf-8"), str(inst_path))
    _APP_INST[name] = inst_path
    return inst_path
