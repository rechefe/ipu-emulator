...
runs layernorm_64x192, qk_scores_64x48, attn_v_64x48, attn_scores_km_64x48,
attn_v_bcast_48, residual_add_64x192, proj_*_p4, or any softmax kernel.

Identical in every particular to fixture_packed_l5_measure.py (the L5
packed-layout session's own instruction-counting wrapper) -- this part of
the harness has zero L4-specific content, so it is a straight copy, not a
re-derivation. See that file's docstring for the full rationale.
