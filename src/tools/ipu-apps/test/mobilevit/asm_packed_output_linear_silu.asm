...
# asm_packed_output_linear_generic.asm except the final store applies
#
# Layer:   L5
# Scope:   single-stream
# Layout:  packed (input, output) -- scatter-on-write construction
# Shape:   Kch->8ch x 16tok, packing factor 8 (K parameterized)
# Related: asm_packed_output_linear_generic.asm (identity-activation base this varies);
#          asm_packed_output_linear_silu_p4.asm (P4 variant)
# Tests:   exercised via test_full_layer_l5_packed.py
#
# ACTIVATE.QUANTIZE silu instead of identity, matching every production
# FFN1 kernel (proj_ffn1_*_p4.asm). GELU is not used by any kernel in this
# codebase (it exists only as an activation-fn ID in the ISA), so this
# kernel uses silu to match production.
#
# Packed-OUTPUT linear layer, GENERIC (K, N_OUT=8 fixed) -- K up to 240
# (30 packed chunks) and beyond. N_OUT is fixed at 8 per call: with N_OUT=8 the
# 8 R_MASK slots map 1:1 onto p_out, which is the alignment the construction
# depends on (the 8 R_MASK slots map 1:1 onto the 8 p_out values -- an
# alignment that holds only at P8, which is L5). For N_OUT>8
...
# here. K assumed a multiple of 8 (packing group size); the L5 layer's
...
# partitions 1-7 unless the caller explicitly reloads R_MASK first.
