...
# both the peeled first chunk and the runtime loop body. One store per chunk
...
{%- for p_out in range(8) %}
    ...
    {#- p_out=7's load+row_idx/BLT all fuse into ONE bundle: write_ptr,
        ...
        241-loads-for-240-channels count, verified empirically): the
        load's result is never consumed by another MULT, since the loop
        exits right after. #}
    MULT.RC.VE {{ rc_idx_reg }} {{ ONE }} {{ p_out }} {{ mask_shift0 }} {{ DSTRUCT128 }};
    ACC.ADD;
    LDR_CYCLIC_MULT_REG {{ read_ptr }} {{ ZERO }} {{ slot0_lr }};
