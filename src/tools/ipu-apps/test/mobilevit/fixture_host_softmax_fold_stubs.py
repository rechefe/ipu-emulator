...
`test_full_layer_l4.py` / `test_full_layer_l5.py` use them to close a whole
layer end to end (with USE_REAL_SOFTMAX=1 the softmax stage runs the real
softmax kernels instead). Nothing in this module runs on the IPU emulator;
it is pure host numpy, deliberately named so it cannot be mistaken for a
kernel: no `App` class, no `setup`/`teardown`, no `.asm`, and it does not
live under `src/ipu_apps/`.
