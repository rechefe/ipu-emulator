...
channel axis; softmax uses the unmodified softmax kernel). That
sub-chain uses the PRODUCTION unpacked App classes
(QkScores16x60App/AttnV16x60App), each in its own dedicated IpuState, fed
the raw bytes read from the shared state's XMEM through the harness's own
setup() (see the "Stage 2" section comment).
...
as a known host-side boundary rather than silently worked around. See the "no host-side operation" audit printed at
the end of the test.
...
read-only (their binaries come from the kernel registry via
fixture_kernel_support.kernel_inst). The packed kernels are test-local .asm
programs next to this file (asm_packed_*.asm); their shared state is a plain
FP32 wide-vector IpuState since no registered harness describes them.
"""
...
from ipu_apps.kernels.attention.qk_scores_16x60.app import QkScores16x60App
from ipu_apps.kernels.attention.attn_v_16x60.app import AttnV16x60App
...
_PACKED_LAYERNORM_ASM = Path(__file__).parent / "asm_packed_layernorm_240x16.asm"
_UNPACK_ASM = Path(__file__).parent / "asm_packed_unpack_240x16.asm"
_PACK_ASM = Path(__file__).parent / "asm_packed_pack_240x16.asm"
_LINEAR_IDENTITY_ASM = Path(__file__).parent / "asm_packed_output_linear_generic.asm"
_LINEAR_SILU_ASM = Path(__file__).parent / "asm_packed_output_linear_silu.asm"
_RESIDUAL_ASM = Path(__file__).parent / "asm_packed_residual_add_240x16.asm"
...
class RowAllocator:
    ...
    arithmetic (a common source of off-by-N bugs) by tracking a single running cursor."""
...
def run_packed_residual_add(state: IpuState, *, a_base_row: int, b_base_row: int,
    ...
    a real chaining hazard, exposed by this test's "single IpuState, no
    host touch" requirement, that no single-kernel test would ever catch.
    Handled here by explicitly restoring the all-ones mask before every call,
    rather than modifying the (validated) production kernel.
    """
    assert a_base_row == 0, (
...
#
# QkScores16x60App / AttnV16x60App keep their XMEM regions at fixed rows
# (K always at row 0: K_BASE comes from the hardwired-zero cr0). Rather than
# share the main chain's IpuState (which would collide at row 0 with this
# chain's own packed data), this sub-chain runs each kernel in its OWN
# dedicated IpuState (the harness's make_state()), and moves bytes between it
# and the main chain's state via plain Python byte slicing/concatenation --
# NOT numpy arithmetic, no dtype conversion. The bytes reach the dedicated
# state through the harness's OWN setup() (a temp file holding exactly the
# raw bytes read from the main state's XMEM), so its staging and register
# programming are the production harness's, not a hand copy of them; the
# result is read back straight from the dedicated state's XMEM.
# ---------------------------------------------------------------------------
...
def run_qk_scores_one_head(main_state: IpuState, *, q_unpacked_base_row: int,
    ...
    cycles). Uses a dedicated IpuState (see the section comment above)."""
    import ipu_apps.kernels.attention.qk_scores_16x60.app as qk
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        q_path = Path(tmpdir) / "q.bin"
        k_path = Path(tmpdir) / "k.bin"
        q_path.write_bytes(q_bytes)
        k_path.write_bytes(k_bytes)
        app = QkScores16x60App(inst_path=kernel_inst("qk_scores_16x60"),
                               query_path=q_path, key_path=k_path)
        qk_state, cycles = app.run(max_cycles=200_000, state=app.make_state())
...
def run_softmax_query_major_one_head(scores: np.ndarray) -> np.ndarray:
    ...
    setup()/teardown() perform genuine host-side repacking between a
    row-major file and its own on-device partitioned layout -- there is no
    XMEM-only path into this class, unlike every other App used in this
    chain. Reported as the one unavoidable host-side/file boundary in this
    full-layer chain."""
    with tempfile.TemporaryDirectory() as tmpdir:
        inp = Path(tmpdir) / "in.bin"
        outp = Path(tmpdir) / "out.bin"
        inp.write_bytes(scores.astype(np.float32).tobytes())
        app = SoftmaxRowsPartialApp(inst_path=kernel_inst("softmax_rows_partial"), input_path=inp,
                                    output_path=outp, n=N_TOK, rows=N_TOK)
...
def run_attn_v_all_heads(main_state: IpuState, *, probs_by_head: list[np.ndarray],
    ...
    import ipu_apps.kernels.attention.attn_v_16x60.app as av
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        p_path = Path(tmpdir) / "p.bin"
        v_path = Path(tmpdir) / "v.bin"
        p_path.write_bytes(bytes(p_bytes))
        v_path.write_bytes(v_bytes_to_rows(v_bytes, N_CH))
        app = AttnV16x60App(inst_path=kernel_inst("attn_v_16x60"), p_path=p_path, v_path=v_path)
        av_state, cycles = app.run(max_cycles=200_000, state=app.make_state())
