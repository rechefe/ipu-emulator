...
channels/row (96 rows) -- partition_size(64)=64 gives PACK=128/64=2. Checks
correctness against numpy float64 and reports executed instruction count,
cycles, and XMEM activation bytes, packed (asm_packed_layernorm_192x64.asm)
vs the unpacked layernorm_64x192 kernel, which runs unmodified as the
baseline.
"""
...
_PACKED_ASM_SRC = Path(__file__).parent / "asm_packed_layernorm_192x64.asm"
...
def test_packed_layernorm_correctness_and_cost(tmp_path: Path) -> None:
    ...
    # ---- Unpacked run (layernorm_64x192, the unpacked baseline) ----
    unpacked_bin = kernel_inst("layernorm_64x192")
