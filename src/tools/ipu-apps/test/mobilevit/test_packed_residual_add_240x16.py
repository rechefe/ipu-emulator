"""Packed elementwise residual add, L5 shape: 240 channels x 16 tokens,
PACKED 8 channels/row -- the "easy case" (no cross-partition combine).
Checks correctness and reports executed instruction count, cycles, and XMEM
activation bytes, packed (asm_packed_residual_add_240x16.asm, run directly
on an FP32 wide-vector IpuState) vs the unpacked residual_add_16x240 kernel,
which runs unmodified as the baseline.
"""
...
_PACKED_ASM_SRC = Path(__file__).parent / "asm_packed_residual_add_240x16.asm"
...
def test_packed_residual_add_correctness_and_cost(tmp_path: Path) -> None:
    ...
    # ---- Unpacked run (residual_add_16x240, the unpacked baseline) ----
    unpacked_bin = kernel_inst("residual_add_16x240")
