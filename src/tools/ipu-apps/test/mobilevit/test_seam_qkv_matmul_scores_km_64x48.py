...
one-channel-per-row convention -- see kernels/matmul/matmul_576x192_x128/app.py
...
from ipu_apps.kernels.attention.attn_scores_km_64x48 import app as km
from ipu_apps.kernels.matmul.matmul_576x192_x128 import app as mm

from fixture_kernel_support import POISON, kernel_inst, poison

N, D, P, N_HEAD, LANES = km.N, km.D, km.P, km.N_HEAD, km.LANES

# Q/K/V block boundaries within the 576 QKV-matmul output rows.
_Q_ROW0, _K_ROW0, _V_ROW0 = 0, N_HEAD * D, 2 * N_HEAD * D  # 0, 192, 384
assert _K_ROW0 == 192 and _V_ROW0 == 384 and 3 * N_HEAD * D == mm.N_OUT


def _run_qkv_matmul_one_stream(D_act: np.ndarray, W: np.ndarray, tmp_path: Path, tag: str) -> bytes:
    ...
    poison(state, mm.OUTPUT_BASE, mm.N_OUT)
    state, cycles = app.run(max_cycles=20_000_000, state=state)
    assert cycles > 0

    raw = output_path.read_bytes()
    assert len(raw) == mm.N_OUT * mm.OUTPUT_ROW_BYTES, (
        f"producer output size {len(raw)} != expected {mm.N_OUT * mm.OUTPUT_ROW_BYTES}"
    )
    rows = np.frombuffer(raw, dtype=np.float32).reshape(mm.N_OUT, mm.LANES)
    all_poison_rows = np.all(rows == POISON, axis=1)
...
def _addressing_only_slice(raw_per_stream: dict[int, bytes], row0: int) -> bytes | None:
    ...
    """
    mm_row_bytes = mm.OUTPUT_ROW_BYTES           # 512: matmul's store pitch
...
def _streams(seed):
    ...
    D_acts = {p: rng.uniform(-1.0, 1.0, size=(mm.K, mm.N_TOK)).astype(np.float32) for p in range(P)}
    return W, D_acts


def test_qkv_matmul_output_pitch_blocks_pure_addressing_into_scores_km(tmp_path: Path) -> None:
    ...
    """
    assert mm.OUTPUT_ROW_BYTES == 512
    assert N * 4 == 256
    assert mm.OUTPUT_ROW_BYTES != N * 4, (
    ...
    W, D_acts = _streams(0x9EC0)
    raw_per_stream = {p: _run_qkv_matmul_one_stream(D_acts[p], W, tmp_path, tag=str(p)) for p in range(P)}
...
def test_qkv_matmul_to_scores_km_agrees_after_repack(tmp_path: Path) -> None:
    ...
    already line up.
    """
    W, D_acts = _streams(0x9EC1)
    raw_per_stream = {p: _run_qkv_matmul_one_stream(D_acts[p], W, tmp_path, tag=f"r{p}") for p in range(P)}
    ...
    q_blocks = np.stack([block_tightly_packed(raw_per_stream[p], _Q_ROW0) for p in range(P)])
    k_blocks = np.stack([block_tightly_packed(raw_per_stream[p], _K_ROW0) for p in range(P)])

    q_path = tmp_path / "q_repacked.bin"
    k_path = tmp_path / "k_repacked.bin"
    q_path.write_bytes(q_blocks.reshape(P, N_HEAD, D, N).astype(np.float32).tobytes())
    k_path.write_bytes(k_blocks.reshape(P, N_HEAD, D, N).astype(np.float32).tobytes())
    output_path = tmp_path / "scores_out.bin"

    head = 1
    app = km.AttnScoresKM64x48App(
        inst_path=kernel_inst("attn_scores_km_64x48"),
        input_path=q_path,
        weights_path=k_path,
        output_path=output_path,
        head=head,
    ...
    state = app.make_state()
    poison(state, km.SBASE, km.S_ROWS)
    state, cycles = app.run(max_cycles=20_000_000, state=state)
    assert cycles > 0

    got = np.frombuffer(output_path.read_bytes(), dtype=np.float32).reshape(P * N, LANES)[:, :N]
