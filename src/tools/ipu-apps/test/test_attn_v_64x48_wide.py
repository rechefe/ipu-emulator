...
from ipu_apps.attention.attn_v_64x48 import (
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "attn_v_64x48.bin"
...
def test_attn_v_64x48_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
...
def test_attn_v_64x48_wide_fp32_padding_is_inert(inst_file: Path, tmp_path: Path) -> None:
