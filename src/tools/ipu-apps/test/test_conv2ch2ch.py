"""End-to-end regression tests for the 2ch->2ch convolution application.
...
_CONV2CH2CH_INST_BIN = Path(os.environ["CONV2CH2CH_INST_BIN"])
_CONV2CH2CH_DATA_DIR = Path(os.environ["CONV2CH2CH_DATA_DIR"])


def _run_conv2ch2ch(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 2ch->2ch conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _CONV2CH2CH_DATA_DIR / dtype_dir
    ...
    app = Conv2ch2chApp(
        inst_path=_CONV2CH2CH_INST_BIN,
...
def test_conv2ch2ch(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_conv2ch2ch(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _CONV2CH2CH_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
