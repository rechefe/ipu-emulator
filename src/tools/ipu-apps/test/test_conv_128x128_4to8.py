"""End-to-end regression tests for the standard 4->8 channel convolution app.
...
_CONV4TO8_INST_BIN = Path(os.environ["CONV4TO8_INST_BIN"])
_CONV4TO8_DATA_DIR = Path(os.environ["CONV4TO8_DATA_DIR"])


def _run_conv4to8(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 4->8ch std conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _CONV4TO8_DATA_DIR / dtype_dir
...
def test_conv_128x128_4to8(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_conv4to8(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _CONV4TO8_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
