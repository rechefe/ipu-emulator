"""End-to-end regression tests for the 32x32 96->96 channel convolution app.
...
from ipu_apps.conv_32x32x96 import Conv32x32x96App


_CONV_INST_BIN = Path(os.environ["CONV32X32X96_INST_BIN"])
_CONV_DATA_DIR = Path(os.environ["CONV32X32X96_DATA_DIR"])


def _run_conv(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 32x32 96->96 conv app for a given dtype, return (output_bytes, cycles)."""
...
def test_conv_32x32x96(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
