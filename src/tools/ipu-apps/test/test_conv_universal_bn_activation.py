...
class TestConvUniversalBnActivation:
    ...
    def test_same_asm_runs_in_wide_vector_debug_mode(
        ...
        This is the mode-blindness property the row-addressed ISA buys us: XMEM
        operands are row numbers and r_cyclic operands are element indices, so
        neither depends on the 1 B/element vs 4 B/element width. Only the *data*
        the harness lays down widens (input and kernel; the mask does not, since
        it is 1 bit per lane in both modes).

        With ``wide_vector_quantize_output`` the wide path re-quantizes to INT8
        in the same leading lane positions as narrow, so the two modes are
        directly comparable -- here, bit-exact against the same reference.
        """
        ...
        bias = rng.randint(-8, 9, size=out_ch).astype(np.int8)

        # Input packs 4 bytes/element in wide mode, same ELEMENT layout as narrow.
        rows_per_chunk = 128 // cols
        num_chunks = (rows * cols) // 128
