...
from ipu_apps.qk_scores_16x60 import QkScores16x60App, N, D, ROW_BYTES
...
_QK_SRC = Path(__file__).resolve().parents[1] / "src/ipu_apps/qk_scores_16x60/qk_scores_16x60.asm"
_SM_SRC = Path(__file__).resolve().parents[1] / "src/ipu_apps/softmax/softmax_rows_partial/softmax_rows_partial.asm"
_KM_SRC = Path(__file__).resolve().parents[1] / "src/ipu_apps/attn_scores_km_16x60/attn_scores_km_16x60.asm"
...
def test_decisive_uncropped_feed(tmp_path: Path) -> None:
    ...
    assemble_to_bin_file(_QK_SRC.read_text(), str(qk_bin))

    reset_labels()
    sm_bin = tmp_path / "sm.bin"
    assemble_to_bin_file(_SM_SRC.read_text(), str(sm_bin))
...
def test_decisive_uncropped_feed_key_major(tmp_path: Path) -> None:
    ...
    assemble_to_bin_file(_KM_SRC.read_text(), str(km_bin))

    reset_labels()
    smc_bin = tmp_path / "smc.bin"
    assemble_to_bin_file(_SMC_SRC.read_text(), str(smc_bin))
