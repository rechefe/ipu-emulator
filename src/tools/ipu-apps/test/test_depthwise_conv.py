"""End-to-end regression tests for the depthwise convolution application.
...
_DCONV_INST_BIN = Path(os.environ["DCONV_INST_BIN"])
_DCONV_DATA_DIR = Path(os.environ["DCONV_DATA_DIR"])


def _run_dconv(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run depthwise conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _DCONV_DATA_DIR / dtype_dir
...
def test_depthwise_conv(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_dconv(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _DCONV_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
