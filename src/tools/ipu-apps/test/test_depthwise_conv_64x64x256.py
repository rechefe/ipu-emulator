"""End-to-end regression tests for the 256-channel depthwise convolution app (64x64).
...
_DCONV256_INST_BIN = Path(os.environ["DCONV256_INST_BIN"])
_DCONV256_DATA_DIR = Path(os.environ["DCONV256_DATA_DIR"])


def _run_dconv256(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 256-ch depthwise conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _DCONV256_DATA_DIR / dtype_dir
...
def test_depthwise_conv_64x64x256(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_dconv256(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _DCONV256_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
