"""Wide-vector FP32 end-to-end test for matmul_128x64x64.
...
N=64 < LANES, so each output row occupies a whole 512 B XMEM row of which only
the first N*4 bytes are valid; the harness teardown crops them, leaving a
densely packed M x N output file.
"""
...
from ipu_apps.matmuls.matmul_128x64x64 import MatMul128x64x64App, M, K, N
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "matmul_128x64x64.bin"
...
def test_matmul_128x64x64_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
    rng = np.random.RandomState(0x646464)
