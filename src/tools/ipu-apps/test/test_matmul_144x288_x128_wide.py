"""Wide-vector FP32 end-to-end test for matmul_144x288_x128.
...
from ipu_apps.matmuls.matmul_144x288_x128 import (
    MatMul144x288x128App, K, N_OUT, N_TG, N_TOK, LANES,
)
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "matmul_144x288_x128.bin"
        assemble_to_bin_file(ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


def test_matmul_144x288_x128_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
