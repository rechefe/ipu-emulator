...
from ipu_apps.matmuls.matmul_384x192_x128 import (
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "matmul_384x192_x128.bin"
        assemble_to_bin_file(ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


def test_matmul_384x192_x128_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
