...
channels/row (30 rows). Measures correctness against numpy float64,
executed instruction count, cycles, and XMEM activation bytes, packed vs
the existing unpacked layernorm_16x240 kernel.

Standalone/throwaway: no BUILD.bazel target. layernorm_16x240 is used
...
_PACKED_ASM_SRC = Path(__file__).resolve().parent / "asm_packed_layernorm_240x16.asm"
_UNPACKED_ASM_SRC = (
...
def test_packed_layernorm_correctness_and_cost(tmp_path: Path) -> None:
    ...
    # ---- Unpacked run (existing layernorm_16x240, read-only baseline) ----
    reset_labels()
