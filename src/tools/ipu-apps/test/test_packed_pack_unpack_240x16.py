"""On-chip pack/unpack kernels for the attention seam: packed (8 channels/
row, 30 rows) <-> unpacked (one channel per row, 240 rows), 240 channels x
16 tokens (L5 shape). Measures correctness and cost for both directions.

Standalone/throwaway: no BUILD.bazel target. Does not modify or import
qk_scores_16x60, attn_v_16x60, or any softmax kernel.
"""
...
_UNPACK_ASM_SRC = Path(__file__).resolve().parent / "asm_packed_unpack_240x16.asm"
_PACK_ASM_SRC = Path(__file__).resolve().parent / "asm_packed_pack_240x16.asm"
...
def test_unpack_correctness_and_cost(tmp_path: Path) -> None:
    ...
    print(f"UNPACK_CYCLES={cycles}")
    print(f"UNPACK_INSTRUCTIONS={counts.total}")
    print(f"UNPACK_ACTIVATION_BYTES={activation_bytes}")
...
def test_pack_correctness_and_cost(tmp_path: Path) -> None:
    ...
    print(f"PACK: cycles={cycles} instrs={counts.total} err={err:.6e} "
          f"activation_bytes={activation_bytes}")
    print(f"PACK_CYCLES={cycles}")
    print(f"PACK_INSTRUCTIONS={counts.total}")
    print(f"PACK_ACTIVATION_BYTES={activation_bytes}")

    assert err < 1e-5, f"pack wrong: max abs error {err:.6e}"


def test_pack_unpack_roundtrip(tmp_path: Path) -> None:
    """pack(unpack(X)) == X and unpack(pack(X)) == X, chained on-chip with
    no host-side conversion between the two kernel runs (only XMEM reads/
    writes by the harness, which is legal load-time staging, not a
    conversion between two kernels' data already resident in XMEM)."""
