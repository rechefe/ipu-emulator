"""End-to-end regression tests for the 128x128x64→16 pointwise convolution application.
...
_PCONV128_INST_BIN = Path(os.environ["PCONV128_64TO16_INST_BIN"])
_PCONV128_DATA_DIR = Path(os.environ["PCONV128_64TO16_DATA_DIR"])


def _run_pointwise_conv(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 128x128x64→16 pointwise conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _PCONV128_DATA_DIR / dtype_dir
...
def test_pointwise_conv_128x128_64to16(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_pointwise_conv(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _PCONV128_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
