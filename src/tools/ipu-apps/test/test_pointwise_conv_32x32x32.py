"""End-to-end regression tests for the 32x32x32 pointwise convolution application.
...
_PCONV32_INST_BIN = Path(os.environ["PCONV32_INST_BIN"])
_PCONV32_DATA_DIR = Path(os.environ["PCONV32_DATA_DIR"])


def _run_pointwise_conv(tmp_path: Path, dtype_dir: str, dtype_str: str) -> tuple[bytes, int]:
    """Run 32x32x32 pointwise conv app for a given dtype, return (output_bytes, cycles)."""
    data_dir = _PCONV32_DATA_DIR / dtype_dir
...
def test_pointwise_conv_32x32x32(tmp_path: Path, dtype_dir: str, dtype_str: str, golden_name: str) -> None:
    actual, cycles = _run_pointwise_conv(tmp_path, dtype_dir, dtype_str)
    assert cycles > 0
    golden = _PCONV32_DATA_DIR / dtype_dir / golden_name
    if golden.exists():
