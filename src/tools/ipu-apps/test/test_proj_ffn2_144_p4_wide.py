...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "proj_ffn2_144_p4.bin"
        assemble_to_bin_file(ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


def test_proj_ffn2_144_p4_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
