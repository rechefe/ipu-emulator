"""Wide-vector FP32 end-to-end test for proj_qkv_240_p4.
...
from ipu_apps.projections.proj_qkv_240_p4 import (
    ProjQKV240P4App, K, N_OUT, N_TOK, N_STREAM, LANES,
)
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "proj_qkv_240_p4.bin"
        assemble_to_bin_file(ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


def test_proj_qkv_240_p4_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
    ...
    for p in range(N_STREAM):
        ...
        np.testing.assert_allclose(
            got[:, :N_TOK], expected,
            rtol=1e-4, atol=1e-3,
            err_msg=f"proj_qkv_240_p4 stream {p} output mismatch",
        )
