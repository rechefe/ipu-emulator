...
from ipu_apps.residual_add.residual_add_16x240 import (
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "residual_add_16x240.bin"
...
def test_residual_add_16x240_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
    a, b, output_path = _run(inst_file, tmp_path)
...
def test_one_channel_per_row(inst_file: Path, tmp_path: Path) -> None:
