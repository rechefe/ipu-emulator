...
from ipu_apps.residual_add.residual_add_256x144 import (
...
def inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "residual_add_256x144.bin"
        assemble_to_bin_file(ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


def test_residual_add_256x144_wide_fp32(inst_file: Path, tmp_path: Path) -> None:
