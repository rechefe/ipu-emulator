"""End-to-end regression tests for the residual_mlp application.
...
_INST_BIN = Path(os.environ["RESIDUAL_MLP_INST_BIN"])
_DATA_DIR = Path(os.environ["RESIDUAL_MLP_DATA_DIR"])
...
def test_residual_mlp(tmp_path: Path, stage: str) -> None:
    """x + gamma_2 * mlp(norm2(x)), summed at INT32 and clamped to INT8."""
...
@pytest.mark.parametrize("stage", ["stage3", "stage4"])
def test_residual_mlp_exercises_the_int8_clamp(tmp_path: Path, stage: str) -> None:
