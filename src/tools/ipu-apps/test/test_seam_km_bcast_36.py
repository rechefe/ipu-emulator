...
from ipu_apps.attention.attn_scores_km_256x36 import (
...
def km_inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "attn_scores_km_256x36.bin"
...
def _run_attn_scores_km(
    km_inst_file: Path, tmp_path: Path, q_path: Path, k_path: Path, head: int, tag: str,
) -> bytes:
...
def test_seam_attn_scores_km_to_attn_v_bcast_36(
    ...
    per_head_raw = [
        _run_attn_scores_km(km_inst_file, tmp_path, q_path, k_path, head=h, tag=f"h{h}")
...
def test_seam_km_bcast_head_block_pitch_mutation_detected(
    km_inst_file: Path, bcast_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    per_head_raw = [
        _run_attn_scores_km(km_inst_file, tmp_path, q_path, k_path, head=h, tag=f"mut{h}")
        for h in range(N_HEADS)
    ]
