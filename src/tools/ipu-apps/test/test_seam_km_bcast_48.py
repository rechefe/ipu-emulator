...
from ipu_apps.attention.attn_v_bcast_48 import (
...
def bcast_inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "attn_v_bcast_48.bin"
...
def _run_km_producer(
    km_inst_file: Path, q: np.ndarray, k: np.ndarray, head: int, tmp_path: Path,
) -> bytes:
...
def test_seam_km_bcast_48_agrees(
    ...
    head_outputs = {
        h: _run_km_producer(km_inst_file, Q, K, h, tmp_path) for h in range(N_HEAD)
    }
...
def test_seam_km_bcast_48_row_pitch_mismatch_is_caught(
    km_inst_file: Path, bcast_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    head_outputs = {h: _run_km_producer(km_inst_file, Q, K, h, tmp_path) for h in range(N_HEAD)}
