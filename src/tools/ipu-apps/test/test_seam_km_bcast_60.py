...
def bcast_inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assemble_to_bin_file(_BCAST_ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path
...
def _run_km_scores(
    km_inst_file: Path, tmp_path: Path, Q: np.ndarray, K: np.ndarray, head: int, tag: str,
) -> bytes:
    ...
    """
    from ipu_apps.attention.attn_scores_km_16x60 import SBASE, OUTPUT_ROW_BYTES, N_TOK as _KM_N_TOK, N_TG as _KM_N_TG
...
def _run_attn_v_bcast(
    bcast_inst_file: Path, tmp_path: Path, p_bytes: bytes, v_buf: np.ndarray, tag: str,
) -> np.ndarray:
    ...
    """
    from ipu_apps.attention.attn_v_bcast_60 import OBASE, O_CHAN_BYTES
...
def _build_chained_p(
    km_inst_file: Path, tmp_path: Path, Q: np.ndarray, K: np.ndarray,
) -> bytes:
    ...
    for h in range(N_HEAD):
        raw = _run_km_scores(km_inst_file, tmp_path, Q, K, head=h, tag=f"h{h}")
...
def test_seam_km_to_bcast_60_agrees(
    km_inst_file: Path, bcast_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    p_bytes = _build_chained_p(km_inst_file, tmp_path, Q, K)
    ...
    got = _run_attn_v_bcast(bcast_inst_file, tmp_path, p_bytes, v_buf, tag="chained")
...
def test_seam_km_to_bcast_60_row_pitch_and_head_concat(
    ...
    raw_h0 = _run_km_scores(km_inst_file, tmp_path, Q, K, head=0, tag="pitchcheck")
...
def test_seam_km_to_bcast_60_mutation_kills_test(
    km_inst_file: Path, bcast_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    got = _run_attn_v_bcast(bcast_inst_file, tmp_path, bytes(mutated), v_buf, tag="mutated")
