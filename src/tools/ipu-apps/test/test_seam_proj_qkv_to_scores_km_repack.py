...
from ipu_apps.proj_qkv_192_p4 import (
...
from ipu_apps.proj_qkv_240_p4 import (
...
_ATTN_SCORES_KM_64X48_INST_BIN = Path(os.environ["ATTN_SCORES_KM_64X48_INST_BIN"])
_PROJ_QKV_240_P4_INST_BIN = Path(os.environ["PROJ_QKV_240_P4_INST_BIN"])
_ATTN_SCORES_KM_16X60_INST_BIN = Path(os.environ["ATTN_SCORES_KM_16X60_INST_BIN"])
...
def test_repack_control_mismatch_is_detected(tmp_path: Path) -> None:
    ...
    app = AttnScoresKM64x48App(
        inst_path=_ATTN_SCORES_KM_64X48_INST_BIN,
...
def test_proj_qkv_240_p4_to_scores_km_16x60_agrees_l5(tmp_path: Path) -> None:
    ...
    for p in range(QKV5_P):
        ...
        app = AttnScoresKM16x60App(
            inst_path=_ATTN_SCORES_KM_16X60_INST_BIN,
            input_path=q_path, weights_path=k_path, output_path=output_path, head=head,
        )
        state, cycles = app.run(max_cycles=20_000_000, state=state)
        assert cycles > 0
