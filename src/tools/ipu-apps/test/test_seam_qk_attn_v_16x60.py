...
from ipu_apps.attention.qk_scores_16x60 import (
...
def qk_inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "qk_scores_16x60.bin"
        assemble_to_bin_file(_QK_ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path


@pytest.fixture(scope="module")
def attn_v_inst_file():
    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "attn_v_16x60.bin"
        assemble_to_bin_file(_ATTN_V_ASM_PATH.read_text(encoding="utf-8"), str(path))
        yield path
...
def _run_qk_scores(
    qk_inst_file: Path, tmp_path: Path, q_head: np.ndarray, k_head: np.ndarray, tag: str,
) -> bytes:
    ...
    """
    from ipu_apps.attention.qk_scores_16x60 import S_BASE, OUTPUT_ROW_BYTES, N as _QK_N, N_TG as _QK_N_TG
...
def _run_attn_v(
    attn_v_inst_file: Path, tmp_path: Path, p_bytes: bytes, v_buf: np.ndarray, tag: str,
) -> np.ndarray:
    ...
    """
    from ipu_apps.attention.attn_v_16x60 import OBASE, O_CHAN_BYTES
...
def _build_chained_p(
    qk_inst_file: Path, tmp_path: Path, Q: np.ndarray, K: np.ndarray,
) -> bytes:
    ...
    for h in range(N_HEAD):
        ...
        k_head = K[lo:lo + D]
        raw = _run_qk_scores(qk_inst_file, tmp_path, q_head, k_head, tag=f"h{h}")
...
def test_seam_qk_to_attn_v_16x60_agrees(
    qk_inst_file: Path, attn_v_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    p_bytes = _build_chained_p(qk_inst_file, tmp_path, Q, K)
    ...
    got = _run_attn_v(attn_v_inst_file, tmp_path, p_bytes, v_buf, tag="chained")
...
def test_seam_qk_to_attn_v_16x60_head_concat_order(qk_inst_file: Path, tmp_path: Path) -> None:
...
def test_seam_qk_to_attn_v_16x60_mutation_kills_test(
    qk_inst_file: Path, attn_v_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    got = _run_attn_v(attn_v_inst_file, tmp_path, bytes(mutated), v_buf, tag="mutated")
...
def test_seam_qk_to_attn_v_16x60_shared_state_zero_copy(
    qk_inst_file: Path, attn_v_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    """
    from ipu_apps.attention.qk_scores_16x60 import K_ROWS
    from ipu_apps.attention.attn_v_16x60 import (
