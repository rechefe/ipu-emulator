...
from ipu_apps.attention.qk_scores_256x36 import QkScores256x36App, N, D, N_TG, N_TPG, LANES
...
def _run_qk_scores(
    ...
    from ipu_apps.attention.qk_scores_256x36 import S_BASE, OUTPUT_ROW_BYTES
...
def test_seam_qk_scores_to_attn_v_256x36(
    ...
    for h in range(N_HEAD):
        raw = _run_qk_scores(qk_inst_file, tmp_path, Q[h], K[h], tag=f"h{h}")
    ...
    from ipu_apps.attention.attn_v_256x36 import OBASE, O_CHAN_BYTES
    poison_row = (np.full(LANES, POISON, dtype=np.float32)).tobytes()
...
def test_seam_qk_attn_v_head_block_pitch_mutation_detected(
    qk_inst_file: Path, attn_v_inst_file: Path, tmp_path: Path,
) -> None:
    ...
    per_head_raw = [
        _run_qk_scores(qk_inst_file, tmp_path, Q[h], K[h], tag=f"mut{h}") for h in range(N_HEAD)
    ]
