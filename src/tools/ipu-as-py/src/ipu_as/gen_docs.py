...
OPERAND_TYPE_DETAILS: dict[str, str] = {
    ...
    "MultMaskOffsetImmediate": (
        ...
        "The value in the **`mask_shift`** LR register is interpreted as a signed index "
        "**`mask_shift_idx` ∈ [−3, +3]** (values outside this range are clamped; LR registers "
        "are 20-bit so negative indices use 20-bit two's complement). It selects from seven masks "
        "generated by sequentially shifting the base mask and ANDing with the **partition vector** "
        "at each step:\n\n"
        ...
        "- `partition = P` (`P > 0`): P partitions of `128/P` elements each. "
        "A **0** bit marks the start of each partition; **1** bits mark elements within a partition. "
        "Example for `P = 2`: `0 1`⁶³` 0 1`⁶³.\n\n"
        "The AND at each step prevents mask bits from crossing partition boundaries. "
        "Lanes in **`mult_res`** where the resulting mask bit is **1** are zeroed before accumulation."
    ),
