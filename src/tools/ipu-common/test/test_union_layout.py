...
def test_all_slots_present():
    ...
    expected = {"load", "store", "debug", "lr", "mult", "acc", "aaq", "cond", "break"}
    assert set(SLOT_UNIONS.keys()) == expected


def test_debug_slot_marked_non_hardware():
    """The debug slot is flagged simulation-only in SLOT_METADATA."""
    assert SLOT_METADATA["debug"]["hardware"] is False
    assert is_hardware_slot("load")
    assert is_hardware_slot("store")
    assert not is_hardware_slot("debug")
