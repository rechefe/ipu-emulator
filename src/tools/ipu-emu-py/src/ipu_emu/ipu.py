...
from ipu_common.registers import get_register_sizes, get_lane_count, get_mult_stage_map
...
def _active_lane_count(mask_int: int, valid_elements: int) -> int:
    """Lanes that both pass the multiply mask and lie within ``valid_elements``.
    ...
    carry real data, so statistics honour it. Lanes 0..n-1, matching
    ``_agg_active_lane_count``'s contiguous-from-zero semantics.
    ...
    return (mask_int & ((1 << n) - 1)).bit_count()
