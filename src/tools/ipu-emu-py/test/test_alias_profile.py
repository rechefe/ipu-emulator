...
def test_constant_exp_and_fractional(declared):
    def setup(state):
        data = struct.pack("<f", math.log2(math.e)) * 128
        state.xmem.write_address(0, data)
        if declared:
            state.xmem.mark_profile_constant(0, data)
    state = run("""
...
""", setup=setup, wide=True)
    status = "verified" if declared else "candidate"
    assert count(state, "A9_EXP", status) == 1
    assert count(state, "A10_FRACTIONAL_SCALAR", status) == 1
    assert state.stats.mult_identity_cycles == 0


def test_constant_declaration_revoked_by_same_value_write():
    state = IpuState()
    state.xmem.write_address(0, b"x" * 128)
    state.xmem.mark_profile_constant(0, b"x" * 128)
    state.xmem.write_address(1, b"x")
    assert not state.xmem.is_profile_constant(0, b"x" * 128)
...
def test_exp_rebase_with_offset_is_candidate_until_offset_is_proven():
    def setup(state):
        state.xmem.write_profile_constant(0, struct.pack('<f', math.log2(math.e)) * 128)
    state = run('''
LDR_MULT_REG R0 LR0 CR0;;
MULT.RC.VV LR0 R0 0 LR0 CR15; ACC.ADD.FIRST;;
...
def test_binary_vector_mask_uses_active_dtype(dtype_name):
    ...
    state.xmem.write_profile_constant(0, bytes([0, dtype_one_byte(dtype)]) * 64)
    load_program(state, [decode_instruction_word(w) for w in assemble('''
LDR_MULT_REG R0 LR0 CR0;;
