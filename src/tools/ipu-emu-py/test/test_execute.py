...
class TestPostAaqReg:
    ...
    def test_str_post_aaq_reg_int8_writes_leading_128_to_xmem(self):
        ...
        state.dtype = DType.INT8
        state.set_cr_dstructure(128)
        # Wide int32 lanes spanning the clamp range; ACTIVATE.QUANTIZE clamps to
        # [-128, 127] and packs the 128 INT8 lanes into post_aaq_reg[0:128].
        buf = bytearray(512)
        struct.pack_into("<128i", buf, 0, *[i - 64 for i in range(128)])
        state.regfile.set_r_acc_bytes(buf)
        state.regfile.set_cr(8, 0x4000)
        # Pre-fill post_aaq_reg with dummy data to prove ACTIVATE.QUANTIZE
        # overwrites the leading 128 lanes (and zeroes the trailing 384).
